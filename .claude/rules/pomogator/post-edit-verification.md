...
## Коммит — отдельной командой ПОСЛЕ чтения результатов

ЗАПРЕЩЕНО чейнить `… && git commit` за тест-раном в одной команде: коммит
уходит раньше, чем прочитан хвост. Инцидент 54b42e7 (2026-06-07): закоммичено
«verdict GREEN» при реальном RED. Порядок: прогнал → ПРОЧИТАЛ хвост →
отдельной командой коммит.
