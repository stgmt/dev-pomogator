...
Валидатор работает в **две фазы**: Phase 1 проверяет структуру (секции, формат), Phase 2 проверяет наличие `### Extracted Requirements` в `## Context`. Phase 2 запускается ТОЛЬКО когда Phase 1 = 0 ошибок. При Phase 2 deny агенту показываются последние промпты пользователя из кэша.
...
1. **Context**
   ...
   - **`### Extracted Requirements`** (ОБЯЗАТЕЛЬНО): нумерованный список ВСЕХ требований, извлечённых из сообщений пользователя в диалоге. Минимум 2 пункта. Формат: `1. ...`, `2. ...`.
   - Перед заполнением: перечитай ВСЕ сообщения пользователя в текущем диалоге.
...
6. **Impact Analysis (обязательно для delete/rename/move)**
...
9. **File Changes (Plan Output) — ОБЯЗАТЕЛЬНО В САМОМ КОНЦЕ**
