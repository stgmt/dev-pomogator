...
Это правило задаёт **единый формат планов разработки**. Если пользователь просит план (или включён Plan mode) — план **обязан** содержать 8 секций ниже и завершаться **непустой** таблицей **File Changes** (что именно будет создано/изменено/удалено и почему).
