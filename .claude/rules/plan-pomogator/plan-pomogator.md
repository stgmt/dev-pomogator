...
2. **Step 2 (дождаться подтверждения)**: AI ждёт ответа от пользователя в свободной форме ("да" / "верно" / "нет, на самом деле X") — это не AskUserQuestion, а обычное conversational подтверждение.

3. **Step 3 (написать план-файл)**: Если пользователь подтвердил — AI пишет полный план-файл в `~/.claude/plans/{slug}.md` с `## 💬 Простыми словами` ПЕРВОЙ секцией (содержимое скопировано из чат-сообщения) перед `## 🎯 Context`. Если пользователь поправил — AI обновляет понимание и повторяет Step 1.
...
**ЗАПРЕЩЕНО**: вызывать ExitPlanMode без выполненного Step 1. Без чат-сообщения с пониманием задачи перед монстр-планом фича не работает — пользователь не сможет проверить понимание AI до того как уже инвестировано усилий в FR/AC/Todos.
