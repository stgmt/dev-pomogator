...
> **spec-authoring-steer compliance:** when writing a full `{ content }` doc via `apply_spec_change`, put `[skip-spec-steer: spec-phase-finalization autofill]` in the `reason` so the steer hook treats this sanctioned phase authoring as automation, not hand-authoring.
