...
> to enumerate first), then apply the shown regex to the RETURNED content in your
> own context. Never run a raw `grep`/`Read`/`cat` over `.specs/` — that is blocked
> under enforce and defeats the audit log. Greps over **code** (`src/`, `tools/`)
> for external-claim / naming checks stay ordinary `Grep`. For @feature tag and
> coverage questions prefer the graph tools (`find_by_tags`, `get_coverage_summary`,
> `conformance_check`) — they already parsed every doc.
...
1. `read_spec_doc({ spec: "{slug}", doc })` for `FR.md`, `USE_CASES.md`, `ACCEPTANCE_CRITERIA.md` (MCP-rails — not a raw Read of `.specs/`).

2. Read source истины:
   - Plan file (если есть): `~/.claude/plans/{name}.md` → `### Extracted Requirements` (outside `.specs/` → ordinary Read)
   - Jira artifacts: `read_spec_doc({ spec: "{slug}", doc: "JIRA_SOURCE.md" })` если есть
...
```
read_spec_doc({ spec: "{slug}", doc: "RESEARCH.md" })   # MCP-rails — not grep/awk over .specs/
# from the returned content:
#   unchecked open questions → lines matching ^- \[ \]
#   the Open Questions section → the block from `## Open Questions` to the next `## `
```
...
```
# Prefer the GRAPH (it already parsed every tag): find_by_tags / get_coverage_summary
# / conformance_check surface @featureN inheritance + orphan/untagged mismatches.
# If you need the raw per-file tag sets, read each doc through the door (MCP-rails):
#   for doc in USER_STORIES.md USE_CASES.md FR.md NFR.md ACCEPTANCE_CRITERIA.md REQUIREMENTS.md TASKS.md:
#     read_spec_doc({ spec: "{slug}", doc }) → extract @feature[0-9]+ , sort -u
#   for the .feature: list_spec_docs({ spec }) → read_spec_doc the *.feature → extract @feature[0-9]+
```
...
```
# MCP-rails: read the docs through the door, then apply these regexes to the content.
# list_spec_docs({ spec: "{slug}" }) → read_spec_doc each *.md , then scan for:
#   PowerShell в bash blocks: (\bTest-Path\b|\$env:|Get-ChildItem|New-Item|Set-Location|Write-Host|\$_\.|Where-Object)
# read_spec_doc({ spec: "{slug}", doc: "TASKS.md" }) , then scan for:
#   raw test runners: \b(dotnet test|npm test|npx vitest|pytest|cargo test|go test)\b
#   raw shell tools:  ^\s*(\$ )?(find|grep|cat|head|tail|sed|awk)\s
```
...
3. Read TASKS.md → Phase 0 секцию (`## Phase 0: Test Infrastructure` или подобное):
   ...
   ```
   read_spec_doc({ spec: "{slug}", doc: "TASKS.md" })   # MCP-rails
   # from the content, take the block from `## Phase 0` to the next `## `
   ```
...
4. Read FILE_CHANGES.md:
   ...
   ```
   read_spec_doc({ spec: "{slug}", doc: "FILE_CHANGES.md" })   # MCP-rails
   # scan returned content for: (create|edit).*hook
   ```
...
1. Identify proposed names в spec:
   ...
   ```
   # MCP-rails: read each via the door, then extract the type names.
   # for doc in DESIGN.md SCHEMA.md FILE_CHANGES.md:
   #   read_spec_doc({ spec: "{slug}", doc }) → match (enum|class|interface|record|struct)\s+\w+ , take the name, sort -u
   ```
