...
const isDirectRun =
  process.argv[1]?.endsWith('audit-rules.ts') ||
  process.argv[1]?.endsWith('audit-rules.js') ||
  // Backward compat — invocations via previous audit.ts path (FR-9)
  process.argv[1]?.endsWith('audit.ts') ||
  process.argv[1]?.endsWith('audit.js');
if (isDirectRun) main();

export { main as runRulesAudit };
