...
| Marketplace/self-hosted агент описан только public policy, но не private deployment registry: `runtimeBaseUrl` то утекал в браузер, то пропадал из модели; live route/policy считались proof без backend dispatch/auth-money-path | create-spec workflow не заставлял разделять public manifest/policy и private runtime registry на Phase 1/2 | `create-spec` cross-boundary checklist: manifest/policy ≠ deployment registry; `runtimeBaseUrl` private backend config; public DTO exposes `runtimeConfigured`; FR/AC/BDD/TASKS обязаны включать redaction + backend dispatch/readiness + auth/balance/idempotency/live proof |
