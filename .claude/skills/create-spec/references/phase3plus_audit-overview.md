...
1. Создать `.specs/{feature}/AUDIT_REPORT.md` через mutation-дверь (MCP-rails, FR-40 — НЕ raw `Write`; под enforce raw запись в `.specs/**` блокируется): `apply_spec_change({ spec: "{feature}", doc: "AUDIT_REPORT.md", content: "<по шаблону references/templates/AUDIT_REPORT.md.template из create-spec skill>", reason: "phase3+ audit report" })`. Дверь валидирует форму перед записью + пишет в аудит-лог.
