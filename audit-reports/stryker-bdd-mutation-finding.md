...
Conclusion: with deterministic tests + deterministic `scan()`, the only remaining variable is **how stryker's `@stryker-mutator/cucumber-runner` attributes per-test coverage and detects kills** — i.e. it intermittently runs the wrong (or no) covering scenario for a mutant, scoring a killable mutant as "survived". The per-mutant flip list (flip-diff, run B in progress) will name the exact mutants; the fix is to NOT gate on the cucumber-runner aggregate (use the deterministic inject+restore unit), or to switch off perTest for the BDD config (full-suite per mutant — slower but each mutant sees every covering scenario, removing the attribution race).
