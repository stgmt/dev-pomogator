...
if command -v gh >/dev/null 2>&1; then
    ...
    if [ -z "$GH_AUTH_TOKEN" ] && [ -f "/workspaces/dev-pomogator/.mcp.json" ]; then
        GH_AUTH_TOKEN=$(jq -r '.mcpServers | to_entries[] | select(.value.env.GITHUB_TOKEN != null) | .value.env.GITHUB_TOKEN' "/workspaces/dev-pomogator/.mcp.json" | head -n 1)
