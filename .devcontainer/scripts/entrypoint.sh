...
find "/workspaces/dev-pomogator/.devcontainer/scripts" -name '*.sh' -exec sed -i 's/\r$//' {} + 2>/dev/null || true

# Fix git worktree .git file (Windows → Linux path)
GIT_FILE="/workspaces/dev-pomogator/.git"
if [ -f "$GIT_FILE" ]; then
...
if [ -f "/workspaces/dev-pomogator/.devcontainer/scripts/start-gui.sh" ]; then
    bash "/workspaces/dev-pomogator/.devcontainer/scripts/start-gui.sh" > /tmp/start-gui.log 2>&1 &
fi

# Auto-snapshot
if [ -f "/workspaces/dev-pomogator/.devcontainer/scripts/auto-snapshot.sh" ]; then
    bash "/workspaces/dev-pomogator/.devcontainer/scripts/auto-snapshot.sh" >> /tmp/auto-snapshot.log 2>&1 &
fi
