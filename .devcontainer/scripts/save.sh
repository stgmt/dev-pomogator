...
IMAGE_TAG="dev-pomogator-snapshot:${BACKUP_NAME}"
...
docker commit \
    --message "dev-pomogator devcontainer snapshot: ${BACKUP_NAME}" \
...
if docker volume inspect "dev-pomogator-home" &>/dev/null 2>&1; then
    docker run --rm \
        -v dev-pomogator-home:/vol_data/dev-pomogator-home \
        alpine tar czf - /vol_data 2>/dev/null > "${TMP_DIR}/volumes.tar.gz"
    echo "Home volume сохранён"
else
    echo "   dev-pomogator-home — не найден, пропускаю"
fi
