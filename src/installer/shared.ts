 ...
 */
export function isDevPomogatorCommand(command: string): boolean {
  return (
    command.includes('.dev-pomogator/tools/') ||
 ...
 */
const REQUIRED_SCRIPTS = new Set<string>([
  'tsx-runner.js',
 ...
 */
async function copyBundledScript(distDir: string, scriptsDir: string, srcName: string, destName?: string, fallbackPaths?: string[]): Promise<void> {
  ...
  if (REQUIRED_SCRIPTS.has(srcName)) {
    ...
    throw new Error(
      `Fatal: ${srcName} not found — run "npm run build" first. ` +
...
export async function setupGlobalScripts(distDir: string): Promise<void> {
  ...
  if (!runnerExists) {
    throw new Error(
      `Post-install verification failed: ${runnerPath} missing after setupGlobalScripts. ` +
      `This leaves hooks broken. Re-run installer after "npm run build".`
    );
  }
  if (!bootstrapExists) {
    throw new Error(
      `Post-install verification failed: ${bootstrapPath} missing after setupGlobalScripts. ` +
