...
function isOldFormat(command: string): boolean {
...
export async function migrateOldProjectHooks(platform: 'claude' | 'cursor'): Promise<number> {
  ...
  let config: { installedExtensions?: Array<{ platform: string; projectPaths?: string[] }> };
  try {
  ...
  for (const ext of extensions) {
    if (ext.platform !== 'claude') continue;
    for (const p of ext.projectPaths ?? []) projectPaths.add(p);
  }
 ...
 */
function migrateProjectSettings(projectPath: string): number {
