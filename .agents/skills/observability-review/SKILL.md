...
dev-pomogator's runtime signals are scattered: escape-hatch JSONL logs in `.Codex/logs/`,
...
|------|--------|------------------|
| **Escape hatches** | `.Codex/logs/*-escapes.jsonl` | a gate bypassed; recent reasons. SHORT/repeated reasons (`ok`, `skip`, `fix`) = gaming → read the diff that was waved through |
...
- `.Codex/rules/scope-gate/escape-hatch-audit.md` + siblings — per-gate gaming guidance.
