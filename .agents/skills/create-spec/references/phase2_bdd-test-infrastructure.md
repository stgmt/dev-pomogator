...
> **Независимо от классификации ниже** — каждый написанный сценарий обязан проходить проверку силы (инвариант на требование + негативных/краевых не меньше, чем happy-path + мысленная подмена в коде + никаких `[TBD]`-заготовок). Это применяется ко ВСЕМ фичам, в том числе `TEST_DATA_NONE`. Критерии и расшифровка — [`feature-creation-rules.md`](feature-creation-rules.md) §6. Слабый сценарий, который зеленит сломанный код, — это не покрытие.
