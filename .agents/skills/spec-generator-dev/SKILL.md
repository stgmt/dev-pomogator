...
1. **Найди производителя.** Грепни уникальную сигнатуру артефакта («[TBD title]», «### Citations»,
   формат сообщения находки) по `tools/` и `.Codex/skills/*/scripts/` — у каждого генератора
   есть отпечаток.
...
| Escape-hatch аудит | `.Codex/logs/{spec-access,spec-variant-matrix,spec-architecture,scope-gate}-escapes.jsonl` | JSONL append-only | каждое использование escape-маркера (anti-gaming) | `tail`/`jq` + правила `*/escape-hatch-audit.md` |
...
6. **Плагин-дистрибуцию МОЖНО тестировать — НЕ заявляй «нельзя прогнать установку».** Инцидент 2026-06-08: сказал юзеру «установку не прогнать», а тулзы есть: `Codex plugin validate <path> [--strict]` (валидирует манифест; `--strict` варнит нераспознанные поля — так подтверждается что новое поле типа `userConfig` реально), `Codex --plugin-dir <path>` (грузит плагин из папки на сессию — локальный тест без marketplace), `Codex --plugin-url`, `Codex plugin details/list/install/init`. Перед утверждением о поведении плагина — прогони `validate`/`--plugin-dir`, не догадывайся.
7. **MCP/guard, меняющий поведение у ЮЗЕРОВ, верифицируй ЖИВЫМ агентом (`Codex -p`), а не только смоуком сервера.** Методика P17-6: stamp размера `spec-access.jsonl` → `[SPEC_ACCESS_ENFORCE=true] Codex -p --allowedTools "mcp__dev-pomogator-specs__* …" -- "<естественная спек-задача>"` → дельта лога. 0 сырых Read/Grep = агент пошёл в дверь. Естественная задача (не «use the MCP tool» — это театр). Голый `Codex -p` classifier пропускает; `--dangerously-skip-permissions` и self-widening settings — НЕТ (действия юзера).
...
- Правило: `.Codex/rules/spec-verdict/no-structural-valid.md` (FR-37d)
