...
2. **Triage** each finding into one of three buckets:
   ...
   - **Known false-positive** (log + skip): e.g. audit-spec `PARTIAL_IMPL_DETECTION` on FR-1 matching the BMAD tier name "Deferred" (not a real partial impl). Append to `.Codex/logs/arch-review-accepted.jsonl` `{ts, check, reason}` and do NOT treat as blocking. Do not invent false-positives to dodge real work.
...
- Skill under review: `.Codex/skills/architecture-decision-builder/SKILL.md`
