...
description: |
  ...
  to `.Codex/logs/cross-spec-overrides.jsonl` for audit trail.
...
line to `.Codex/logs/cross-spec-overrides.jsonl` so audit can trace
...
```bash
node --import tsx .Codex/skills/cross-spec-reconcile/scripts/reconcile-cli.ts \
...
|---|---------|---------|----------------|
| 1 | **WRAP-deprecated** | `impl-drift/missing-file` or `missing-symbol`; referenced path removed in a major migration with NO canonical replacement | Reference is load-bearing for traceability (spec/ownership table); pure deletion would erase migration history. Example: `src/installer/Codex.ts` → `~~src/installer/Codex.ts~~ (removed in v2 — no canonical replacement)` |
