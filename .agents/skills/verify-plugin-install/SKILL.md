...
description: |
  ...
  Docker container, fully non-interactively, using the real `Codex plugin` CLI. Use when asked to
...
Codex 2.1.x ships a full **non-interactive plugin CLI** — the interactive `/plugin` TUI is NOT
the only path. The whole `marketplace add → install → load` flow is scriptable, so a clean-room Docker
test is achievable. Verified working 2026-05-27 (Codex 2.1.152). This caught a real bug: a `plugin.json`
...
```
Codex plugin validate <path>                      # validate a plugin OR marketplace manifest
Codex plugin marketplace add <path|repo|url>      # add marketplace (local DIRECTORY path works!)
Codex plugin install <name>@<marketplace> -s user # install (scope: user|project|local)
Codex plugin list --json                          # confirm installed + enabled
Codex plugin uninstall <name>@<marketplace>
Codex --plugin-dir <path> ...                     # load a plugin from a dir for ONE session (quick smoke)
```

Real on-disk layout (NOT `enabledPlugins`/`extraKnownMarketplaces` in settings.json — that does
nothing): `~/.Codex/plugins/{installed_plugins.json, known_marketplaces.json, marketplaces/, cache/}`.
...
`Codex plugin marketplace validate` but makes `Codex plugin install` fail with
...
"skills": ".Codex/skills", "hooks": ".Codex-plugin/hooks.json"
// ✅ correct
"skills": ["./.Codex/skills"], "commands": ["./.Codex/commands"],
"hooks": ["./.Codex-plugin/hooks.json"], "mcpServers": ["./.mcp.json"]
```

Official marketplace plugins set NONE of these (they auto-discover from conventional `skills/`,
`commands/`, `hooks/hooks.json`, `.mcp.json`). dev-pomogator keeps everything under `.Codex/` +
`.Codex-plugin/`, so it MUST declare the custom paths — as arrays.

## The Docker recipe (the harness lives in tests/e2e + .dev-pomogator-tmp/plugin-e2e)

1. Image: `FROM node:22-slim` + `npm i -g @anthropic-ai/Codex` + `useradd tester`.
2. Bind-mount the repo read-only at `/plugin`. Run as `--user tester` (NON-root — `--dangerously-skip-permissions`
   is refused under root).
3. In-container: `Codex plugin validate /plugin` → `marketplace add /plugin` → `install dev-pomogator@stgmt -s user`
   → `plugin list --json`.
4. Headless load check: `Codex -p "Reply OK" --dangerously-skip-permissions --debug --output-format stream-json --verbose`.
...
- Real `Codex -p` makes a live billed LLM call (~$0.5 with a 1M context model). Keep the prompt one word.
- Steps 1–4 (validate/add/install/list) need NO auth — only the final `Codex -p` load check does.
...
(gitignored; canonical install does not `npm install`). So any hook (`.Codex-plugin/hooks.json`),
...
**lazy-import + fail-open**, or **rewrite node-builtins-only**. See `.Codex/rules/testing/dead-integration-guard.md`
...
on-demand deep check (needs Codex CLI + auth), driven by this skill.

## Honest limits
- The literal interactive `/plugin` TUI (browse/trust prompts) isn't automatable — but `Codex plugin
...
## Sources
Verified empirically 2026-05-27 with Codex 2.1.152: `Codex plugin --help`, `Codex plugin validate`
