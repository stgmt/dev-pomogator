...
```bash
npx tsx .Codex/skills/spec-reality-check/scripts/verify.ts .specs/{slug} --format human
```
...
```bash
npx tsx .Codex/skills/spec-reality-check/evals/run-evals.ts        # 25/25 must pass
npx tsx .Codex/skills/spec-reality-check/evals/bulk-run.ts         # surface false positives on real corpus
npx tsx .Codex/skills/spec-reality-check/evals/bench-synthetic.ts  # NFR ≤30s; algorithm O(N)
```

3. If `regression-baseline-pinned` evals fail OR `bulk-run` clean-spec count decreases → unfixed regression; DO NOT commit.

Full discipline + anti-patterns in `.Codex/rules/spec-reality-check/maintain-evals-on-edit.md` (always-apply rule).
