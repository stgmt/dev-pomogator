---
name: "source-command-suggest-rules"
description: "Анализ сессии и предложение Codex rules на основе выявленных паттернов"
---

# source-command-suggest-rules

Use this skill when the user asks to run the migrated source command `suggest-rules`.
...
Проведи ретроспективный анализ текущей сессии и предложи Codex rules для сохранения ценного контекста и паттернов.
...
> **Codex only.** Использует skill `/deep-insights` для глубокого анализа кросс-сессионных данных.
...
Skill выполняет:
- Агрегацию ALL facets JSON (~50+ файлов) из `~/.Codex/usage-data/facets/`
- Кросс-корреляцию friction↔outcome
- Анализ tool errors и workflow patterns
- Чтение report.html для AGENTS.md suggestions и big wins
...
| `AGENTS.md Suggestions` | suggestion text + priority | HIGH priority pattern candidates |
...
Для каждого AGENTS.md suggestion с priority HIGH:
```
📊 Insights находка: "<suggestion text>"
   Секция: AGENTS.md Suggestions
...
```javascript
Read("~/.Codex/usage-data/report.html")
```
...
| AGENTS.md suggestions | `.Codex-md-item` | `data-text` + `.cmd-why` |
...
**Источники доменов:**
1. Имена существующих rules (имена файлов внутри `.Codex/rules/`).
...
| ⚡ **Rule Violations** (T4) | Codex нарушил существующее правило из `.Codex/rules/` |
| 🩹 **Workarounds** (T6) | Обходное решение из-за отсутствия документации |
| 🆕 **New Patterns** (T1) | Codex создал новую утилиту/компонент/helper |
...
│   │   Glob `.Codex/skills/*/SKILL.md` + match по trigger phrases / mission / steps.
│   │
│   ├── ДА → 🔀 SKILL_MERGE — добавить в `.Codex/skills/{existing}/SKILL.md`
...
│
├── Требует суждения Codex / контекстного решения?
...
|----------------|--------------------|---------------|
| тесты, test runner, vitest/pytest/jest invocation, test-guard hook, host-vs-docker bypass, test wrapper invocation | `run-tests` | `.Codex/skills/run-tests/SKILL.md` |
| spec файлы, `.specs/{slug}/`, FR/AC/TASKS/feature workflow, audit categories | `create-spec` | `.Codex/skills/create-spec/SKILL.md` |
| исследование библиотек, MCP/web search, hypothesis verification | `research-workflow` | `.Codex/skills/research-workflow/SKILL.md` |
| screenshot, visual verification, TUI/UI debug | `debug-screenshot` | `.Codex/skills/debug-screenshot/SKILL.md` |
| docker-compose, Dockerfile optimization | `docker-optimize` | `.Codex/skills/docker-optimize/SKILL.md` |
| variant matrix, polymorphic FR detection | `variant-matrix-build` | `.Codex/skills/variant-matrix-build/SKILL.md` |

**Эвристика:** если кандидат — про command-line invocation / configuration / workflow / "как правильно вызывать X" — **всегда** Glob `.Codex/skills/*/SKILL.md` ПРЕЖДЕ чем предлагать rule/checklist.

**Порог evidence** (из Codex Coach):
...
├── Тип: 🎯 skill
├── Название: <kebab-case>
├── Путь: .Codex/skills/<name>/SKILL.md
├── Суть: <1 предложение>
...
**REUSABILITY подсказка:** domain-specific паттерн = полный балл (30), если домен есть в `Project domains` из Phase 0.5. `.Codex/rules/` — это именно место для проектных знаний, domain-specific НЕ штрафуется.
...
1. **Glob** `.Codex/skills/*/SKILL.md` — список существующих skills с trigger phrases.
...
1. Glob `.Codex/skills/*/SKILL.md` — проверить существующие skills
2. Сравнить trigger phrases и workflow steps
3. Если совпадение >80% → DUP, 30-80% → MERGE (добавить reference/step)

**Для hook-кандидатов:**
1. Read `.Codex/settings.json` — проверить существующие hooks
...
|--------|------------|----------|
| 🆕 NEW | <30% (rule-rule), no skill match | Создать новый файл в `.Codex/rules/...` |
| 🔀 MERGE | 30-80% (rule-rule) | Дополнить существующий rule (смежный аспект) |
| 🔀 SKILL_MERGE | match с existing skill | Дополнить `.Codex/skills/{name}/SKILL.md` (НЕ создавать rule) |
...
- **Путь:** `.Codex/rules/antipatterns/no-direct-prod-db.md`
...
| Тип | Путь |
|-----|------|
| 🔴 antipattern | `.Codex/rules/antipatterns/<name>.md` |
| 🟢 pattern | `.Codex/rules/patterns/<name>.md` |
| 📋 checklist | `.Codex/rules/checklists/<name>.md` |
| ⚠️ gotcha | `.Codex/rules/gotchas/<name>.md` |
| 🎯 skill (новый) | `.Codex/skills/<name>/SKILL.md` |
| 🔀 SKILL_MERGE | дополнить `.Codex/skills/<existing>/SKILL.md` (Edit/Append, не Write) |
| 🪝 hook (spec) | `.Codex/rules/hooks/<name>-hook.md` |
| 📁 project-specific | `.Codex/rules/<domain>/<name>.md` |
...
> После создания правил в Phase 5 — автоматически оптимизировать ВСЕ rules в `.Codex/rules/`.
...
```
resolve-library-id("Codex") -> query-docs(id, "rules frontmatter paths format")
```
...
```bash
npx tsx .Codex/skills/skills-rules-optimizer/scripts/audit.ts --dir .Codex/rules --save audit_before.json
```
...
```bash
npx tsx .Codex/skills/skills-rules-optimizer/scripts/audit.ts --dir .Codex/skills --save audit_skills.json
```
...
```bash
npx tsx .Codex/skills/skills-rules-optimizer/scripts/detect-overlap.ts --dir .Codex/skills
```
...
```bash
npx tsx .Codex/skills/skills-rules-optimizer/scripts/check-antipatterns.ts --dir .Codex/rules
```
...
```bash
npx tsx .Codex/skills/skills-rules-optimizer/scripts/audit.ts --dir .Codex/rules --save audit_after.json
npx tsx .Codex/skills/skills-rules-optimizer/scripts/report.ts --before audit_before.json --after audit_after.json
```
...
|---|---------|---------|-----------|-------------|
| T1 | New Pattern Introduced | Low | Codex создал новую утилиту/helper | 🟢 pattern |
| T2 | User Correction | High | Юзер исправил подход Codex (≥2 раз = HIGH) | 🔴 antipattern / ⚠️ gotcha |
| T3 | Repeated Confusion | Medium | Codex спрашивал/сомневался 2+ раз по одной теме | 🟢 pattern / ⚠️ gotcha |
| T4 | Rule Violation | High | Codex нарушил правило из `.Codex/rules/` | 🟢 pattern (уточнение) |
| T5 | Convention Discovered | Low | Codex нашёл undocumented конвенцию через чтение кода | 🟢 pattern |
