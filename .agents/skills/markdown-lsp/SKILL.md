...
name: markdown-lsp
description: >
  How & why to use the Markdown LSP (Marksman, registered as a NATIVE Codex
...
**native Codex LSP plugin** (`.lsp.json` → `plugin.json` `lspServers`), so
its primitives are exposed through Codex's built-in **`LSP` tool** — which
...
- **Fix** — `node tools/anchor-integrity/fix.mjs --spec .specs/<slug> --apply`
  ...
  links it can't disambiguate are left flagged for the `Codex -p` fallback.
