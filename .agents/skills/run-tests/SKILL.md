...
description: >
  ...
  + YAML status tracking — survives Codex Bash tool bg capture drops on Windows.
...
If `.Codex/rules/docker-only-tests.md` exists in the project, tests MUST run through Docker. Automatically add `--docker` flag unless already specified.
...
After launching tests with `run_in_background: true`, start a Monitor to get real-time failure notifications and progress updates in the chat. This lets Codex react to failures immediately instead of waiting for the entire test suite to finish.
...
**When to use Monitor:** Always use Monitor when running tests in background. It adds zero overhead (read-only YAML polling) and enables Codex to start investigating failures while remaining tests are still running.
...
В dev-pomogator существует `tests/setup/ensure-docker.ts` который throw-ит при запуске вне Docker. Причина — большинство e2e тестов dеструктивны: удаляют `~/.Codex/`, `~/.dev-pomogator/`, `.Codex/settings.json` через `setupCleanState()`. На host это убьёт активную сессию пользователя.
...
- Любой `*-installer*.test.ts`, `Codex-installer.test.ts` — destructive
- Тесты с `setupCleanState()` / `initGitRepo()` / writes в `~/.Codex/`
...
| Делает `fs.remove(appPath(...))` или пишет в `~/.Codex/` | ❌ Docker |
...
не ловит (это не test-команда). Ограничение: только pure-логика — функция, трогающая `~/.Codex`/
...
Cross-link: `.Codex/skills/strong-tests/SKILL.md` для skill workflow + thresholds (default 70% kill rate для critical paths).
