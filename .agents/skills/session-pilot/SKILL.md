...
name: session-pilot
description: |
  Worktree dashboard plugin для Codex пользователей с 10+ ворктри.
  Триггеры (RU): "открой dashboard", "покажи мои ворктри", "список worktree",
  "launch Codex в worktree X", "ресуми клода в ворктри Y", "создай worktree для Z",
  "перезапусти session-pilot", "проверь dashboard работает", "продиагностируй lm-saas LIVE".
  Триггеры (EN): "open worktree dashboard", "list my worktrees", "launch Codex in worktree X",
  "resume Codex in worktree Y", "create worktree for Z", "restart session-pilot",
  "diagnose live detection".

  Использовать when:
  (1) пользователь хочет открыть dashboard на http://localhost:8083 ИЛИ
  (2) автоматизировать worktree creation + Codex launch ИЛИ
  ...
  - debugging Codex в общем — это про dashboard, not Codex itself
...
> (зарегистрирован в `.Codex-plugin/hooks.json` — уезжает с плагином на любую машину — И в
> `.Codex/settings.json` — dogfood) сам поднимает сервер на каждой сессии, кроссплатформенно
...
## Scenario 2: Launch Codex --resume в worktree
...
**Когда триггерится**: «launch Codex в worktree X», «resume Codex в feature/auth», «продолжи сессию в lm-saas».
...
3. Если нет Codex history → AskUserQuestion: «Нет UUID для resume. Запустить fresh Codex?»
4. Если есть → `curl -fsS "http://localhost:8083/api/Codex?path=<path>" | jq '.claude_sessions[0].uuid'` — взять последний UUID
...
6. **Verification (mandatory)**:
   ...
   - `mcp__claude-in-chrome__screenshot` — visual confirmation Codex is bootstrapped
...
3. Анализировать output:
   ...
   - **Youngest JSONL > threshold** → Codex batches writes; `LIVE_THRESHOLD_SEC=600 python3 server.py` для retry, OR document baseline behavior to user
...
**Шаги**:
1. `pwsh -File tools/session-pilot/create-launcher.ps1` — создаёт Desktop `.lnk` → hidden `launch.ps1` (single-instance) со своей иконкой `session-pilot.ico` + AppUserModelID `Codex.SessionPilot`.
...
- Codex batches JSONL writes ~2-3 min — default LIVE threshold 300s, NOT 90s.
...
- `.Codex/rules/session-pilot/action-button-injection.md` — write-chars vs new-layout decision tree
- `.Codex/rules/session-pilot/Codex-projects-encoding.md` — path encoding gotchas
- `.Codex/rules/session-pilot/perf-budget.md` — latency targets per endpoint
- `.Codex/rules/session-pilot/mcp-chrome-only.md` — verification must use Codex-in-chrome MCP
- `.Codex/rules/session-pilot/single-instance-launcher.md` — one-window launcher, dedicated-profile detection, 3-place version sync (v0.5+)
