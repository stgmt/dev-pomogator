...
> **Shared base:** общая research-дисциплина (таксономия источников, маркеры верификации, триангуляция, anti-patterns AP-1..8, external-pain, misconception-flush) живёт в [`.Codex/skills/_shared/research-base.md`](../_shared/research-base.md) — один источник правды, общий с `architecture-research-workflow` (его Stage 3 «broad research» зовёт этот навык примитивом). Этот SKILL — конкретный 4-фазный процесс поверх той базы.
...
| AP-2 | **Distribution misassumption** | Предположил «postinstall + npm», реально — `Codex plugin install <name>@<marketplace>` | Distribution всегда — first hypothesis для package research; явно искать `install` команду в docs, не angle inference из npm convention |
...
|----|-----------|---------------------|----------|
| H1 | dev-pomogator can be installed via `Codex plugin install dev-pomogator@stgmt` | CLI command in plugins-reference.md | [UNVERIFIED] if not found |
...
|-----|------------|-----------|
| `code.Codex.com/docs/en/llms.txt` | Sitemap всех docs страниц | YES — daje понимание surface area |
| `code.Codex.com/docs/en/plugins.md` | High-level overview, basic schema | YES |
| `code.Codex.com/docs/en/plugins-reference.md` | **Detailed schema reference** — full field list | YES — критично для AP-1 |
| `code.Codex.com/docs/en/skills.md` | Skill format, frontmatter, scopes | YES для plugin research |
| `code.Codex.com/docs/en/settings.md` | settings.json structure, scopes precedence | YES |
| `code.Codex.com/docs/en/mcp.md` | MCP server scope/lifecycle | If MCP relevant |
| `code.Codex.com/docs/en/discover-plugins.md` | Marketplace mechanics | If marketplace relevant |
| `code.Codex.com/docs/en/hooks.md` | Hooks events + format | If hooks relevant |
...
2. **`site:` оператор** — `site:code.Codex.com plugin enabledPlugins`
...
|-------|----------|---------------|
| **Official docs** | docs.anthropic.com, code.Codex.com | Vendor-authored official documentation |
| **Source code** | github.com/anthropics/Codex | Actual implementation OR examples in official repos |
...
|--------|---------------------|--------|
| `[VERIFIED: <url>]` | 3+ INDEPENDENT angles confirm | `[VERIFIED: plugins-reference.md L370 + github.com/anthropics/Codex/.../config.ts L42 + community blog 2026-01]` |
...
- Official vendor docs (docs.anthropic.com, code.Codex.com и т.д.) — **must be fetched + quoted**
...
|----|-----------|--------|----------------|----------------|---------------------|
| H1 | `Codex plugin install <name>@<m>` is canonical | [VERIFIED] | plugins-reference.md L450 quote: "..." | github/anthropics/.../cli.ts L88 | systemprompt.io/2026/01 blog |
...
| H4 | Desktop reads `~/.Codex/plugins/` | [UNVERIFIED] | not in docs | not testable from docs | user assertion only — risk AP-4 |
```
