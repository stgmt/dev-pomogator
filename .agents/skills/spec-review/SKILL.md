...
| 3 | Antipattern guardrails | P1 | `Grep` `.Codex/rules/antipatterns/*.md` triggers | Phase 2+ |
...
Triggers when project memory dir `~/.Codex/projects/{encoded-cwd}/memory/` contains `feedback_*.md` files. For each such file, extract:
...
**MCP-rails (FR-39/P19-6 — НЕ raw `ls`/`Read` по `.specs/`; под enforce блок):**
обычно slug приходит от вызывающего (`create-spec` передаёт активную спеку). Если slug
известен — сразу `read_spec_doc({ spec, doc: ".progress.json" })`. Если slug НЕ задан
(ручной «проверь спеку») — найди активную через дверь:

```
list_specs()                                        → перечень slug-ов
read_spec_doc({ spec, doc: ".progress.json" })      → для каждого; возьми тот, у кого
                                                       currentPhase != null (активная работа)
```

Прочти `.progress.json` → `currentPhase` определяет scope категорий (см. таблицу выше). Если ни у одной спеки нет активной фазы → завершить с "spec не найдена, нечего ревьюить". (Raw `ls -t` давал mtime-порядок — через дверь его нет; активность определяется по `currentPhase`, не по времени модификации.)
...
- 3 (Antipattern guardrails) — grep по `.Codex/rules/antipatterns/*.md` если существует, иначе skip
...
Записать `REVIEW_NOTES.md` через mutation-дверь (MCP-rails FR-40 — НЕ raw `Write`; под enforce блок): `apply_spec_change({ spec: "{slug}", doc: "REVIEW_NOTES.md", content: "<ниже>", reason: "spec-review notes" })` (overwrite существующий):
...
(Optional. old/new pairs для тривиальных fixes — например, унификация терминологии или замена raw test command на /run-tests.) **MCP-rails (FR-40):** если патч правит `.specs/` документ — применять через `apply_spec_change({ spec, doc, old_string, new_string, reason })` (дверь валидирует форму/анкеры/conformance перед записью), НЕ raw `Edit`. Edit оставить только для файлов вне `.specs/`.
...
- Antipattern rules: `.Codex/rules/antipatterns/*.md` (если существуют в target проекте) — источник triggers для категории #3
