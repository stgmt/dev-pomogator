...
2. **Run the orchestrator** (non-interactive steps):
   ...
   ```
   In the dev-pomogator source repo the path is `.Codex/skills/worktree-setup/scripts/orchestrate.ts`.
...
5. **Read the summary** — the orchestrator ends with a per-step block (created / bootstrapped / env-synced / built /
   devcontainer / doctor / pr) marked ✓ done, ⚠ skipped, ✗ failed, plus the suggested `wt -d <path> Codex` command.
...
6. Bootstrap: canonical Codex context-menu setup uses `scripts/install-codex-context-menu.ps1` from the plugin/repo root. Do not use the deprecated v1 `bin/cli.js` Codex bootstrap for Codex worktrees.
