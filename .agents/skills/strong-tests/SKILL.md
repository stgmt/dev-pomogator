...
npx tsx <dev-pomogator>/.Codex/skills/strong-tests/scripts/run-mutation.ts --analyze-survivors
...
| Path | Purpose |
|---|---|
| `.Codex/skills/strong-tests/SKILL.md` | This file — workflow + 12-point eval |
| `.Codex/skills/strong-tests/scripts/detect-invariant-candidates.ts` | JiT detector — composition-chain, nxm-overlap, collection-returning |
| `.Codex/skills/strong-tests/scripts/run-mutation.ts` | Auto-dispatch Stryker (TS) / mutmut (Python) / Stryker.NET (C#) |
| `.Codex/skills/strong-tests/scripts/survivors-batch-prompt.ts` | v0.5.1: Batch survivors → LLM-ready prompts |
| `.Codex/skills/strong-tests/scripts/merge-survivor-verdicts.ts` | v0.5.1: Merge LLM verdicts back into report |
| `.Codex/skills/strong-tests/references/anti-patterns.md` | 8 anti-patterns catalogue + grep regex |
| `.Codex/skills/strong-tests/references/tooling-setup.md` | Install + thresholds per 6 stacks |
| `.Codex/skills/strong-tests/references/stryker.config.template.mjs` | Template для Stryker (TS) |
| `.Codex/skills/strong-tests/references/stryker-net.config.template.json` | v0.5.0: Template для Stryker.NET (C#) |
...
Документ-в-файле **не** становится behavioural prior автоматически. Real-session incident (session-pilot 2026-05-11): тот же агент написал `.Codex/rules/testing/output-invariants-first.md` и через 2 часа в той же сессии его нарушил — **2 раунда подряд**, оба раза потребовался пинок пользователя. Полный self-postmortem: `dev-pomogator-session-pilot/.specs/session-pilot/POSTMORTEM-test-discipline.md`. Эта секция (§1.5) и есть **prior-activator** — она загружается с каждым load skill body перед §2 Pre-write checklist.
...
npx tsx .Codex/skills/strong-tests/scripts/classify-tests.ts <test-dir>

# Markdown report grouped by category
npx tsx .Codex/skills/strong-tests/scripts/classify-tests.ts <test-dir> --format=markdown

# Lock to specific language (skip auto-detect)
npx tsx .Codex/skills/strong-tests/scripts/classify-tests.ts <test-dir> --language=csharp
```
...
**Решение — Import Guard pattern** (per `.Codex/rules/extension-test-quality.md`):
...
Шаг 3. Integration tests (spawnSync) **остаются** — они верифицируют real CLI invocation contract. Unit tests дают Stryker import chain. **НЕ удаляй integration tests** — это нарушает `.Codex/rules/integration-tests-first.md`.
...
Корневая причина не подтверждена 100% (см. `.Codex/rules/pomogator/tui-debug-verification.md` #3) — три плауsible hypotheses: capture handle drop / pipe buffer race / block-vs-line buffering. Если репро на вашем проекте — open issue в vitest GitHub + cross-link сюда.
...
Trigger: **PostToolUse hook** on `Write|Edit` of production code files. Registered in `.Codex-plugin/hooks.json` (PostToolUse `Write|Edit`). Matchers (v0.1.0): `*.ts` / `*.py`; excludes `*test*` / `__tests__` / `tests/` / `*.test.ts` / `*_test.py` / `*.feature` / `*.md`.
...
1. Эмитит `additionalContext` JSON в stdout per Codex PostToolUse protocol с file path + suspicious function names с line numbers + suggested invariants (cardinality / uniqueness / conservation per `.Codex/rules/testing/output-invariants-first.md` taxonomy)
...
Audit log: `.Codex/logs/strong-tests-skips.jsonl` per `.Codex/rules/scope-gate/escape-hatch-audit.md` analog. Reason ≥8 chars → entry без warning; <8 → entry с `warning: "REASON_TOO_SHORT"` + additionalContext audit note. Schema: `.specs/strong-tests/strong-tests_SCHEMA.md` §"strong-tests-skips.jsonl audit log".
...
The `test-author` subagent (`.Codex/agents/test-author.md`, FR-TA2) runs this recipe in a task's
...
4. **NO blocking the session on mutation runs.** If estimated runtime >2min: instruct caller to use `run_in_background: true` with `tee` persistent log per `.Codex/rules/pomogator/no-blocking-on-tests.md`. The skill MUST emit progress lines for runs >30s (NFR-U1).
...
- **`run-tests`** — centralized test runner with statusline + TUI monitoring. Use `/run-tests` (NOT raw `npm test` / `pytest`) when invoking the test suite from within this skill's workflow per `.Codex/rules/tui-test-runner/centralized-test-runner.md`.
...
- honnibal/Codex-skills (mutation-testing.md.txt + hypothesis-tests.md.txt) — AI-driven manual mutation catalogue (credited)
