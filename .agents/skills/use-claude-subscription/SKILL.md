---
name: use-Codex-subscription
description: |
  Wire up the current project to use the user's Codex subscription via
  the local Anthropic-compatible proxy (Codex-proxy-infra / Meridian on
  ...
  end-to-end with a smoke test. Use whenever the user says "use Codex
  here", "wire Codex in this project", "Codex через подписку",
  "хочу клода в этом проекте без ключа", "анthropic api без ключа",
  "set up Codex api locally", "use my Codex subscription here".
---

# use-Codex-subscription — wire any project to the local Codex subscription proxy

## When to invoke

User says (any of):

- "use Codex in this project" / "set up Codex here"
- "use my subscription instead of API key"
- "wire Codex api into this app"
...
- The local proxy infrastructure is set up (`Codex-proxy-infra` cloned).
  If not, point user to clone it: `git clone <Codex-proxy-infra-url> D:/repos/Codex-proxy-infra`.
...
If multiple → pick the primary one driving Codex calls (usually clear
from grep'ping for `anthropic`/`Codex` in source files).
...
For **OpenAI SDK** clients pointing at Codex (via Meridian's OpenAI-compat):
...
issue, point user to Codex-proxy-infra README troubleshooting.
...
```
Wired <project-name> → http://127.0.0.1:3456 (Codex subscription).
...
llm = ChatAnthropic(model="Codex-sonnet-4-6")  # auto-honors ANTHROPIC_BASE_URL
```
...
- `D:/repos/Codex-proxy-infra/README.md` — user-facing setup docs
- `D:/repos/presentation-reels/.specs/local-Codex-subscription-proxy/` — full spec
