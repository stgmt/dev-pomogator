---
name: "source-command-pomogator-doctor"
description: "Диагностика окружения dev-pomogator: Node/Git/Bun/Python/MCP/hooks/env vars с предложением переустановки"
---

# source-command-pomogator-doctor

Use this skill when the user asks to run the migrated source command `pomogator-doctor`.
...
- 🟡 Needs env vars (`AUTO_COMMIT_API_KEY` и другие required envRequirements в `.env` или `.Codex/settings.local.json → env`)
...
1. Запусти doctor engine: `npx tsx .Codex/skills/pomogator-doctor/scripts/engine/index.ts` через Bash
...
- Skill: `.Codex/skills/pomogator-doctor/SKILL.md`
- Engine: `.Codex/skills/pomogator-doctor/scripts/engine/`

## Migration note (v2.0)

В v1 invoked через `dev-pomogator --doctor` CLI binary. После canonical refactor v2.0 CLI binary deprecated; используется engine directly через `npx tsx <engine>/index.ts` или skill hook (`scripts/doctor-hook.ts` registered как SessionStart hook в plugin's `.Codex-plugin/hooks.json`).
