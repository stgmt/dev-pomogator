...
## Scope Guard — legacy npm only

This skill is only for diagnosing the deprecated v1 `npx`/`npm` installer after the user already attempted that legacy path.

For Codex installs, never suggest `npm`, `npx`, `npx dev-pomogator`, `github:stgmt/dev-pomogator`, or `--Codex`. Use the context-menu install script instead:
