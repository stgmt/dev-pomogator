...
name: proxy-up
description: |
  Ensure the local Codex subscription proxy (Codex-proxy-infra /
  ...
  reports "proxy down", "503 from anthropic", "Codex not responding",
  or wants to bring the proxy up before running an Anthropic SDK app.
  Triggers: "подними proxy", "проверь Codex proxy", "Codex-proxy
  status", "meridian up", "запусти локальный Codex api".
---

# proxy-up — operate the local Codex subscription proxy

## When to invoke

Triggered by any of:

- User asks to start / check / restart the local Codex proxy
...
- User asks "is Codex subscription routing working"

## Conventions

- **Proxy infra location**: ships INSIDE the dev-pomogator plugin at
  `<plugin-root>/tools/Codex-subscription-proxy/` (Dockerfile + compose +
...
1. Resolve infra dir defensively (the proxy infra ships in the plugin tree;
   ...
   [ -d "$PROXY_DIR/scripts" ] || PROXY_DIR="${CLAUDE_PLUGIN_ROOT:-}/tools/Codex-subscription-proxy"
   [ -d "$PROXY_DIR/scripts" ] || PROXY_DIR="$(ls -d "$HOME"/.Codex/plugins/cache/*/dev-pomogator/*/tools/Codex-subscription-proxy 2>/dev/null | sort -V | tail -1)"
   [ -d "$PROXY_DIR/scripts" ] || PROXY_DIR="tools/Codex-subscription-proxy"
   [ -d "$PROXY_DIR/scripts" ] || PROXY_DIR="$HOME/repos/Codex-proxy-infra"
...
2. If still not found (no plugin install, no clone): clone the standalone infra
   ...
   ```bash
   git clone <Codex-proxy-infra-url> "$HOME/repos/Codex-proxy-infra"
   ```
...
- ❌ Suggesting `Codex login` on every failure — only when /health
...
- ❌ Burning user's subscription budget on diagnostic Anthropic API
  ...
  upstream Anthropic.
- ❌ Modifying `~/.Codex/.credentials.json` directly — never. Always
  via `Codex login` or the proxy's `/auth/refresh`.
