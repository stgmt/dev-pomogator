...
description: |
  ...
  прямо в чате. Skill: спрашивает ключ, говорит ГДЕ его взять, либо использует уже настроенный auth
  (`gh auth login`/`gh auth token` для Octocode), затем ВПИСЫВАЕТ секрет в user-global
  `~/.Codex.json` через `set-mcp-key.ts` и РЕАЛЬНО проверяет, что настройка применилась (не вслепую).
...
Секрет пишется ТОЛЬКО в user-global `~/.Codex.json` (не git-tracked) — **никогда** в project
...
```
(или прочитать `~/.Codex.json` `mcpServers` и применить предикаты из `tools/mcp-setup/mcp-auth-detect.ts`).
...
2. **Не предлагать npm/npx как стандартный путь для Codex.** Если пользователь отдельно просит Context7 OAuth-helper,
   сначала проверь актуальную официальную команду Context7; не подставляй старый `npx ... --Codex` из памяти.
...
- Перечитать `~/.Codex.json` и подтвердить, что предикаты теперь true (это делает `set-mcp-key.ts` сам — `"verified":true`).
...
## Запреты
- НЕ писать секрет в project `.mcp.json` — только `~/.Codex.json` (FR-10).
