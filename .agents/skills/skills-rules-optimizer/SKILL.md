...
description: >
  Optimizes .Codex/rules/ AND .Codex/skills/ — audit (token count, frontmatter
  validation, allowed-tools coverage, oversize cap), triple-axis Jaccard overlap
  detection between skills, LLM-driven merge synthesis through Codex
...
Optimize `.Codex/rules/` AND `.Codex/skills/` artifacts:
...
- **Manually**: `audit.ts --dir .Codex/rules` (rules-only) или `audit.ts --dir .Codex/skills` (skills audit + overlap detection)
...
npx tsx .Codex/skills/skills-rules-optimizer/scripts/audit.ts --dir .Codex/rules --save audit_rules.json
...
- `FRONTMATTER_NAME_FORBIDDEN_TOKEN` — name contains "anthropic" / "Codex"
...
```bash
npx tsx .Codex/skills/skills-rules-optimizer/scripts/detect-overlap.ts --dir .Codex/skills [--threshold 0.3]
```
...
```bash
npx tsx .Codex/skills/skills-rules-optimizer/scripts/merge-skills.ts \
...
NO direct Anthropic SDK / API key dependency. Sub-agent invocation entirely через Codex Agent tool.
...
For backward compatibility, when `--dir` matches `.Codex/rules`:

1. **Step 1: Audit** — `audit.ts --dir .Codex/rules --save audit_before.json`
2. **Step 2: Add path-scoped frontmatter** — for each file без `paths`, infer via `references/path-inference-table.md`
3. **Step 3: Fix antipatterns** — `check-antipatterns.ts --dir .Codex/rules`, apply fixes per `references/known-antipatterns.md`
4. **Step 4: Merge small related files** — confirmed same-domain + overlapping paths only
5. **Step 5: Final report** — `audit.ts --dir .Codex/rules --save audit_after.json`, then `report.ts --before audit_before.json --after audit_after.json`
...
- `shinytoyrobots/Codex-skills-linter` (token-cost ranking)
