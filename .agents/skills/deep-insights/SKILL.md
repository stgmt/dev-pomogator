---
name: deep-insights
description: "Deep analysis of Codex usage from /insights data (facets JSON + report.html). Aggregates cross-session friction trends, satisfaction metrics, tool errors, and generates quantitative evidence for rule candidates."
...
Perform deep quantitative analysis of Codex `/insights` data to produce structured analytics output. This skill reads **existing** data from `~/.Codex/usage-data/` — it does NOT generate new insights.

## Data Sources

| Source | Path | Content |
|--------|------|---------|
| Facets JSON | `~/.Codex/usage-data/facets/*.json` | Per-session structured metrics |
| Report HTML | `~/.Codex/usage-data/report.html` | Aggregated visual report |
| Session Meta | `~/.Codex/usage-data/session-meta/*.json` | Detailed session metadata |
...
ls ~/.Codex/usage-data/facets/ 2>/dev/null | wc -l
ls ~/.Codex/usage-data/report.html 2>/dev/null
```
...
```bash
bash !`.Codex/skills/deep-insights/scripts/aggregate-facets.sh`
```
...
Read `~/.Codex/usage-data/report.html` for additional data:
- Big wins section
...
## AGENTS.md Suggestions (from report.html)
