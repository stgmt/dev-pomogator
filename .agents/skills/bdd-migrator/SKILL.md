...
8. **Mutation gutcheck (runtime class).** Break the engine under test, re-run, confirm the scenario
   ...
   (`.Codex/skills/strong-tests/evals/run-evals.ts` is the reusable rubric.)
...
10. **Commit YOURS only — path-limited.** A bare `git commit` commits the WHOLE index incl. a parallel
    ...
    `git commit`. Full discipline in the agent definition `.Codex/agents/bdd-migrator.md`.
...
- **Spawn a FRESH agent PER SPEC — context decay is real.** ATDD-for-Codex
  ...
  dedicated `bdd-migrator` agent PER SPEC (`subagent_type: bdd-migrator`, see `.Codex/agents/bdd-migrator.md`),
