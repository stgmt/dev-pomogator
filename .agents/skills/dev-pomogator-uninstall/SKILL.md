...
If dev-pomogator is installed as a Codex **plugin** (v2, the current model — check with `codex plugin list`), uninstall is one command and the manual steps below are NOT needed:

```bash
codex plugin uninstall dev-pomogator@stgmt   # removes the plugin: skills, rules, hooks, commands
codex plugin marketplace remove stgmt        # optional: also forget the marketplace
```

For Codex v2 reinstall, never suggest the deprecated v1 `npx`/`--Codex` installer. Use the context-menu install script:

```bash
powershell -ExecutionPolicy Bypass -File .\scripts\install-codex-context-menu.ps1
```

The steps below are the **v1 fallback** — only for legacy projects installed by the old installer (managed files under `.dev-pomogator/tools/`, a `.gitignore` marker block, hooks in `.Codex/settings.local.json`, config in `~/.config/dev-pomogator/`). None of that exists in a v2 plugin install.

---

Background: dev-pomogator installs managed files (tools, commands, rules, skills) and writes hooks to `.Codex/settings.local.json`, plus adds a marker block to `.gitignore`. This skill cleanly removes all of that without touching user-authored files or the `~/.dev-pomogator/` global directory (unless user explicitly asks for full removal).
...
> - Delete managed files in `.dev-pomogator/tools/`, `.Codex/rules/`, `.Codex/skills/`, and managed `.Codex/commands/`
> - Strip the dev-pomogator marker block from `.gitignore`
> - Clean dev-pomogator hooks/env from `.Codex/settings.local.json`
...
- `.Codex/rules/<subfolder>/` (after removing all rule files)
- `.Codex/skills/<name>/` (after removing skill dirs)
...
**4e. Clean `.Codex/settings.local.json`.**

Use the **Read** tool to load `.Codex/settings.local.json`. Use the **Edit** tool to remove any hook entries whose `command` field contains:
- `.dev-pomogator/tools/`
...
- `.Codex/rules/` (managed rule files)
- `.Codex/skills/` (managed skill files)
- `.dev-pomogator/`
...
```bash
cat .Codex/settings.local.json 2>/dev/null || echo "file absent (OK)"
```
...
>
> You can reinstall the Codex context-menu later with `powershell -ExecutionPolicy Bypass -File .\scripts\install-codex-context-menu.ps1`. Do not use `npx`/`--Codex` for Codex installs.
...
This reinstalls the canonical Codex plugin path. Do not roll Codex back through the deprecated v1 `npx`/`--Codex` installer.
