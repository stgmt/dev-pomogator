...
0. **Простыми словами** — ОБЯЗАТЕЛЬНО первая секция (перед Context). Top-level секция для human review с тремя подсекциями: `### Сейчас (как работает)`, `### Как должно быть (как я понял)`, `### Правильно понял?`. Содержимое — живой язык без технического жаргона. AI **обязан** вывести содержимое в чат перед ExitPlanMode; явная команда писать/продолжать план уже является подтверждением, поэтому отдельный wait-turn после неё запрещён. См. секцию `## Two-Stage Plan Presentation` ниже.
...
2. **Step 2**: AI сначала определяет, содержится ли подтверждение в текущей сильной директиве. `делай план`, `пиши план`, `составляй`, `продолжай`, `go ahead` и `do it` после summary/ревью уже подтверждают scope; повторный вопрос запрещён. Только при отсутствии директивы и реальной неоднозначности AI ждёт один ответ.
3. **Step 3**: После явной директивы или отдельного подтверждения AI немедленно пишет план-файл с human-summary первой секцией. Поправка пользователя возвращает к Step 1 только если она существенно меняет интерпретацию.
...
### Explicit-directive confirmation bypass

Сильная команда владельца продолжать планирование сама является подтверждением. AI обязан зафиксировать выводимые defaults в Assumptions и действовать; дополнительный confirmation turn допустим только для невыводимого необратимого решения.

**ЗАПРЕЩЕНО**: вызывать ExitPlanMode без выполненного Step 1 или повторно спрашивать подтверждение после явной команды писать план.
