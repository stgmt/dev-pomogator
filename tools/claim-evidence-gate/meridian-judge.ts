...
export interface JudgeInput {
  ...
  userRequest?: string;
  /** Deterministic owner-intent fact from the Stop hook. TRUE means analysis/report is the complete mandate. */
  analysisReportOnlyMandate?: boolean;
  /** FR-28 (2026-06-29): the FULL list of the human's typed requests THIS session (oldest→newest) — the
   *  agent's actual MANDATE. The judge weighs this as a distinct scoring dimension: once EVERY thing the
   *  human asked is done, the remaining nextOpenTask BACKLOG is NOT the agent's job → APPROVE (this OVERRIDES
   *  the next-open-task / fake-handoff rule). Defaults to [userRequest] when unset, so existing callers and
   *  the bench keep their behaviour. Agent-independent — the agent cannot fabricate the human's prompts. */
  sessionUserPrompts?: string[];
...
export function buildJudgePrompt(i: JudgeInput): string {
  ...
  // FR-28: the human's full session MANDATE. Defaults to [userRequest] so existing callers/bench keep
  // their behaviour; empty only when the human typed nothing capturable (then the MANDATE-COMPLETE rule
  // below cannot fire and the normal lazy-stop logic governs).
  ...
  return [
    ...
    '- FAKE HAND-OFF: hands the next ACTION or a task PICK to the user ("скажешь — сделаю", "решать тебе", "жду твоего слова", "назови задачу", "какую взять", "что брать первым", "твой приоритет") to avoid working. When a NEXT OPEN TASK is identified above, asking "which task / which spec should I take or prioritize FIRST?" is NEVER genuine — the agent PICKS that offered task itself and does it. This bans only TASK/SPEC PRIORITIZATION asks — choosing WHICH of the offered open tasks to do first, within ONE spec OR across MULTIPLE specs (owner 2026-06-25 «что брать первым — не вопрос для юзера»). It does NOT ban a genuine DESIGN choice / irreversible trade-off (e.g. «Postgres или Mongo?», «в проде или нет?») — that IS a real owner-decision → APPROVE (see below). EXCEPTION that OVERRIDES this rule — MANDATE COMPLETE: when EVERY item in the human\'s MANDATE (fact above) is already DONE in this session\'s work, the nextOpenTask is unrequested BACKLOG, NOT the agent\'s job — so ending with «done — name the next backlog item or say stop» is the CORRECT close, not a dodge → APPROVE (see MANDATE COMPLETE in the APPROVE list). The nextOpenTask only forces work when it is PART OF the mandate; a backlog task the human never asked for does not.',
