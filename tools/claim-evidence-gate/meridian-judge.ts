...
export function buildJudgePrompt(i: JudgeInput): string {
  ...
  return [
    ...
    '- LEGITIMATELY AWAITING ASYNC: the "background task launched THIS turn" fact is YES, the message is waiting for that result, AND it names NO other concrete next task it could start now — the agent genuinely cannot proceed ("жду результат, сам пока ничего сделать не могу"). WAITING IS NOT A BLANKET LICENSE TO STOP: if it is "waiting" yet NAMES a next task it could take ("возьму следующую — X", "дальше беру Y", "если не скажешь иную приоритетность"), that is announce-and-stop → BLOCK — it must do that non-blocking work now, not idle under cover of the wait.',
    ...
    '',
    'Tie-breaker: named-next (even "X сейчас", and even while "waiting for a background task" — waiting is NOT a stop-license if a next task is named) / deferred-to-later / handed-to-user-to-pick-this-spec\'s-own-work / self-authorized SKIP of doable in-scope work (agent decided to keep/skip on its own, however well-argued) / PAST-tense status-while-open → BLOCK, no matter how many tools ran. APPROVE only: answering-the-user / one genuine owner-decision (design fork / irreversible trade-off / WHICH-SPEC when the session touched multiple specs / owner-directed-or-owner-ASKED skip) / truly-blocked / awaiting-async (bg launched THIS turn) / PRESENT-tense in-flight continuation of the SAME current step / scope-is-ZERO-and-done. Decider when ambiguous: is a CURRENT action explicitly in progress right now (continuation), or is this a finished report / a NEW unit named? in-progress → APPROVE; finished-or-new → BLOCK.',
    '',
