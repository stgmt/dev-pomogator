...
function buildJudgePrompt(i) {
  ...
  return [
    ...
    `- deterministic owner mandate is ANALYSIS/REPORT ONLY (delivery ends the mandate; no implementation implied): ${i.analysisReportOnlyMandate ? "YES" : "no"}`,
    `- THE HUMAN'S FULL SESSION MANDATE \u2014 every distinct thing the human TYPED this session, oldest\u2192newest (agent-independent; the agent CANNOT fake these). The agent's job is THIS mandate, NOT the open backlog of whatever spec it happened to edit:
${mandateBlock}`,
...
${i.finalMessage}`,
    "",
    `STEP 0 \u2014 MANDATE CHECK, DO THIS FIRST, BEFORE the BLOCK and APPROVE lists below: look ONLY at the fact "THE HUMAN'S FULL SESSION MANDATE". If that list is NON-EMPTY and EVERY distinct thing in it is DONE in the work this session shows, AND the only open work left is the nextOpenTask BACKLOG the human did NOT ask for, then APPROVE immediately and STOP \u2014 output block:false now. In that case the message ending with "name the next one or say stop" / "\u043D\u0430\u0437\u043E\u0432\u0438 \u0437\u0430\u0434\u0430\u0447\u0443 \u0438\u043B\u0438 \u0445\u0432\u0430\u0442\u0438\u0442" is the CORRECT close, NOT a fake hand-off: the backlog is NOT the agent's mandate, so the fake-handoff / next-open-task / status-while-open / hand-to-user BLOCK rules below DO NOT APPLY. This OVERRIDES all of them. Proceed to the lists below ONLY if the mandate is NOT fully done OR is "(none captured)". ANTI-GAMING: AND not ANY (every item complete, not just one); judge by the human's ACTUAL typed asks, NOT the agent's paraphrase \u2014 if the work did LESS than the mandate asked (e.g. mandate "audit ALL 26 and fix everything" but the agent only glanced at one), the mandate is NOT complete \u2192 do NOT approve here, fall through to the rules below.`,
    "",
    "DECISION ORDER FOR THE RULES BELOW: apply APPROVE carve-outs first. If one clearly holds, return block:false even when generic BLOCK wording also matches. Otherwise apply BLOCK rules.",
    ...
    `- FAKE HAND-OFF: hands the next ACTION or a task PICK to the user ("\u0441\u043A\u0430\u0436\u0435\u0448\u044C \u2014 \u0441\u0434\u0435\u043B\u0430\u044E", "\u0440\u0435\u0448\u0430\u0442\u044C \u0442\u0435\u0431\u0435", "\u0436\u0434\u0443 \u0442\u0432\u043E\u0435\u0433\u043E \u0441\u043B\u043E\u0432\u0430", "\u043D\u0430\u0437\u043E\u0432\u0438 \u0437\u0430\u0434\u0430\u0447\u0443", "\u043A\u0430\u043A\u0443\u044E \u0432\u0437\u044F\u0442\u044C", "\u0447\u0442\u043E \u0431\u0440\u0430\u0442\u044C \u043F\u0435\u0440\u0432\u044B\u043C", "\u0442\u0432\u043E\u0439 \u043F\u0440\u0438\u043E\u0440\u0438\u0442\u0435\u0442") to avoid working. When a NEXT OPEN TASK is identified above, asking "which task / which spec should I take or prioritize FIRST?" is NEVER genuine \u2014 the agent PICKS that offered task itself and does it. This bans only TASK/SPEC PRIORITIZATION asks \u2014 choosing WHICH of the offered open tasks to do first, within ONE spec OR across MULTIPLE specs (owner 2026-06-25 \xAB\u0447\u0442\u043E \u0431\u0440\u0430\u0442\u044C \u043F\u0435\u0440\u0432\u044B\u043C \u2014 \u043D\u0435 \u0432\u043E\u043F\u0440\u043E\u0441 \u0434\u043B\u044F \u044E\u0437\u0435\u0440\u0430\xBB). It does NOT ban a genuine DESIGN choice / irreversible trade-off (e.g. \xABPostgres \u0438\u043B\u0438 Mongo?\xBB, \xAB\u0432 \u043F\u0440\u043E\u0434\u0435 \u0438\u043B\u0438 \u043D\u0435\u0442?\xBB) \u2014 that IS a real owner-decision \u2192 APPROVE (see below). EXCEPTION that OVERRIDES this rule \u2014 MANDATE COMPLETE: when EVERY item in the human's MANDATE (fact above) is already DONE in this session's work, the nextOpenTask is unrequested BACKLOG, NOT the agent's job \u2014 so ending with \xABdone \u2014 name the next backlog item or say stop\xBB is the CORRECT close, not a dodge \u2192 APPROVE (see MANDATE COMPLETE in the APPROVE list). The nextOpenTask only forces work when it is PART OF the mandate; a backlog task the human never asked for does not.`,
    ...
    '- ANALYSIS/REPORT/RESEARCH/AUDIT/PLAN/REVIEW the user EXPLICITLY asked for: when the deterministic fact "ANALYSIS/REPORT ONLY" is YES, a stop that DELIVERS that analysis/report is the COMPLETE mandate \u2192 APPROVE. It does NOT imply implementation, another pass, or taking the next backlog task \u2014 even if the report identifies fixes. This HOLDS even if it hands a future implementation decision back to the owner. Factual claims still need proof or [UNVERIFIED], but no further WORK is owed. If the fact is no because the owner also asked fix/implement/continue, this carve-out does not apply.',
    `- MANDATE COMPLETE (this OVERRIDES the NEXT-OPEN-TASK / fake-handoff rule above): EVERY distinct thing the human asked this session \u2014 the MANDATE list in the facts \u2014 is DONE in the work this session shows, and the ONLY open work left is the nextOpenTask BACKLOG the human did NOT ask for. Then the agent has finished its assigned mandate; that backlog is NOT its job, and ending with "done \u2014 name the next one or say stop" is the CORRECT close, NOT a fake hand-off \u2192 APPROVE. STRICT, anti-gaming: (a) AND, not ANY \u2014 this holds ONLY if EVERY requested item is complete; if the human asked for X and Y and only X is done \u2192 mandate NOT complete \u2192 the lazy-stop rules apply, BLOCK. (b) It is the human's ACTUAL typed asks that count, NOT the agent's paraphrase \u2014 if the MANDATE list shows a BIGGER ask than the work actually did (e.g. mandate says "audit ALL 26 and fix every issue" but the agent only glanced at one and calls it done), mandate is NOT complete \u2192 BLOCK. (c) If the MANDATE list is "(none captured)", this rule does NOT apply \u2014 fall back to the normal lazy-stop logic.`,
    ...
    `- SKIP/DEFERRAL DIRECTED BY THE OWNER (VERIFIABLE): the skip/deferral is backed by the OWNER'S OWN typed words \u2014 the request explicitly scoped X out OR directed it parked/deferred/blocked/backlog ("\u0434\u0435\u0440\u0436\u0438 X \u0432\u043D\u0435 \u0441\u043A\u043E\u0443\u043F\u0430", "\u043F\u0440\u0438\u043F\u0430\u0440\u043A\u0443\u0439 X \u0432 \u0431\u044D\u043A\u043B\u043E\u0433", "\u043E\u0442\u043B\u043E\u0436\u0438 X", "\u043F\u043E\u043C\u0435\u0442\u044C X BLOCKED", "\u0432\u0435\u0440\u043D\u0451\u043C\u0441\u044F \u043A X \u043F\u043E\u0437\u0436\u0435"). That is the OWNER deciding, verifiable in the fact, not the agent's unverifiable claim \u2192 APPROVE. But: the agent merely ASKING "do X or skip/defer it?" is NOT this carve-out \u2014 doable in-scope work is just DONE, not negotiated down \u2192 BLOCK. And a bare "\u0442\u044B \u0441\u043A\u0430\u0437\u0430\u043B \u043E\u0442\u043B\u043E\u0436\u0438\u0442\u044C" with NO matching owner-words in the fact is an unverifiable claim \u2192 BLOCK.`,
...
var OWNER_DEFERRAL_RE = /припарку\w*|отложи\w*|в\s+бэклог|в\s+backlog|пометь[\s\S]{0,30}(?:block|заблок)|заблокиру\w*|вне\s+скоп|оставь[\s\S]{0,20}(?:вне|на\s+потом)|(?:сделаем|вернёмся)\s+позже|не\s+сейчас|пока\s+не/i;
function ownerDirectedDeferral(ownerWords) {
  return ownerWords.some((w) => typeof w === "string" && OWNER_DEFERRAL_RE.test(w));
}
var ANALYSIS_REPORT_RE = /\bанализ|проанализир|разбер|разбор|исслед|аудит|оцен[иь]|проверь|диагност|отч[её]т|заключени|вывод|рекомендац|\breport\b|analy[sz]|research|investigat|\baudit\b|ревью|\breview\b|\bплан\b|\bplan\b|посмотри\s+что|что\s+думаешь|что\s+не\s+так/i;
var IMPLEMENT_REQUEST_RE = /(?:^|[.!?;]\s*|(?:,\s*)?(?:и|затем|потом|then|and)\s+)(?:почини(?!ть)|исправь|\bfix\b|реализу(?:й|йте)|\bimplement\b|\bbuild\b|мигрируй|\bmigrate\b|допиши|добавь|перепиши|внеси|закоммить|\bcommit\b|запуш|\bpush\b|внедри|устрани|сделай\s+(?:фикс|изменени|правк)|продолж(?:ай|и)|доведи|доделай)/i;
function ownerRequestedAnalysisReportOnly(ownerRequests) {
  ...
  if (!unsupported && isJudgeArmed({
  ...
  })) {
    {
      const jInput = {
        ...
        analysisReportOnlyMandate: analysisOnly,
        // owner-authored closed mandate; delivering the report ends it
        ...
        selfMarkedBlockedOrBacklogThisTurn: selfMarkedBlockedOrBacklogThisTurn && !ownerDirectedDeferral([...sessionPrompts, substantiveUserRequest])
      };
