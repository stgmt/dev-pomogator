...
| `src/scripts/tsx-runner.js` | edit | Insert orphan-detect block immediately after `resolveScriptPath()` (current line 107): if `args[0].startsWith('.dev-pomogator/') && !fs.existsSync(scriptPath)` → append JSONL + dedup-checked stderr hint → `process.exit(0)` — [FR-3](FR.md#fr-3-self-heal-hint-for-orphan-worktrees-via-tsx-runnerjs) |
