...
function parseProgressVersion(p) {
  const obj = JSON.parse(fs2.readFileSync(p, "utf8"));
...
function readProgressVersion(repoRoot, fp) {
  if (fp) {
    ...
    if (slug) {
      const perSpec = path2.join(repoRoot, ".specs", slug, ".progress.json");
      if (fs2.existsSync(perSpec)) return parseProgressVersion(perSpec);
    }
  }
  const global2 = path2.join(repoRoot, ".specs", ".progress.json");
  if (!fs2.existsSync(global2)) return null;
...
function postEditContent(input, repoRoot) {
  ...
  if (tool === "Edit") {
    const absPath = path2.isAbsolute(fp) ? fp : path2.join(repoRoot, fp);
    if (!fs2.existsSync(absPath)) return null;
    const current = fs2.readFileSync(absPath, "utf8");
