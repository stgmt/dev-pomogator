...
function acquireLock(dir, opts) {
  ...
  for (; ; ) {
    try {
      const fd = fs.openSync(lock, "wx", 384);
      ...
      try {
        ...
        if (age > LOCK_STALE_MS) {
          let ownerPid = 0;
          try {
            ownerPid = Number(JSON.parse(fs.readFileSync(lock, "utf8")).pid) || 0;
          } catch {
          }
          let alive = false;
          if (ownerPid > 0) {
...
function normalizeProjectRoot(candidate) {
  if (typeof candidate !== "string" || candidate.trim() === "" || !path2.isAbsolute(candidate)) return null;
  try {
    const canonical = fs2.realpathSync.native(candidate);
    return fs2.statSync(canonical).isDirectory() ? canonical : null;
  } catch {
    return null;
  }
}
...
function projectHasSpecs(projectRoot) {
  ...
  try {
    const specs = path2.join(projectRoot, ".specs");
    const canonical = fs2.realpathSync.native(specs);
    return isWithin2(projectRoot, canonical) && fs2.statSync(canonical).isDirectory();
  } catch {
    return false;
  }
}
function resolveProjectPath(projectRoot, candidate, { mustExist = true } = {}) {
  ...
  const absolute = path2.resolve(root, candidate);
  if (!isWithin2(root, absolute)) return null;
  try {
    const canonical = fs2.realpathSync.native(absolute);
  ...
  while (isWithin2(root, parent)) {
    try {
      const canonicalParent = fs2.realpathSync.native(parent);
...
function shouldGuard(filePath) {
  return classify(filePath) !== null;
}
function specSlugOfPath(fp) {
  const m = fp.replace(/\\/g, "/").match(/(?:^|\/)\.specs\/([^/]+)\//);
  return m ? m[1] : null;
}
function parseProgressVersion(p) {
  const obj = JSON.parse(fs3.readFileSync(p, "utf8"));
  return typeof obj.version === "number" ? obj.version : null;
}
function readProgressVersion(repoRoot, fp) {
  if (fp) {
    const slug = specSlugOfPath(fp);
    if (slug) {
      const perSpec = path3.join(repoRoot, ".specs", slug, ".progress.json");
      if (fs3.existsSync(perSpec)) return parseProgressVersion(perSpec);
    }
  }
  const global2 = path3.join(repoRoot, ".specs", ".progress.json");
  if (!fs3.existsSync(global2)) return null;
  return parseProgressVersion(global2);
}
function makeAllow(reason) {
  return {
    hookSpecificOutput: {
...
function postEditContent(input, repoRoot) {
  ...
  if (tool === "Edit") {
    ...
    if (!absPath || !fs3.existsSync(absPath)) return null;
    const current = fs3.readFileSync(absPath, "utf8");
...
function detectHardFindings(kind, filePath, content, opts = {}) {
  if (kind === null) return [];
  ...
  try {
    if (kind === "md") {
      ...
      for (const anchor of slice.anchors) {
        ...
        if (prevLine !== void 0 && prevLine !== anchor.location.line) {
          denyReasons.push(
...
function runGuard(input, repoRoot, opts = {}) {
  const fp = input.tool_input?.file_path;
  ...
  const version2 = readProgressVersion(repoRoot, fp);
  if (version2 === null || version2 < 4) {
    if (shouldGuard(fp) && typeof version2 === "number") {
      try {
        appendRawEntry(
          {
