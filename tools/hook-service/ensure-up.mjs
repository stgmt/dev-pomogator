...
import { HOST, PORT, VERSION, stateDir, stateFile } from './server.mjs';

const health = async () => { try { const response = await fetch(`http://${HOST}:${PORT}/health`, {signal:AbortSignal.timeout(500)}); const body = await response.json(); return response.status === 200 && body.service === 'dev-pomogator-hook-service' && body.version === VERSION; } catch { return false; } };
...
export async function ensureUp(pluginRoot = process.env.CLAUDE_PLUGIN_ROOT || process.env.CLAUDE_PROJECT_DIR || process.cwd()) {
  ...
  try {
    if (await health()) return {ready:true, port:PORT};
    ...
    if (prior.pid && ownProcess(prior.pid)) {
      ...
      return {ready:false, reason:'owned process is unhealthy', port:PORT};
    }
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) { await new Promise(r => setTimeout(r, 50)); if (await health()) return {ready:true, port:PORT}; }
    return {ready:false, reason:'service did not become healthy', port:PORT};
  } finally {
