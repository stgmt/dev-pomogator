...
**Use Case:** [UC-1](USE_CASES.md#uc-1-happy-path-план-с-simple-summary-в-шапке-feature1), [UC-4](USE_CASES.md#uc-4-backward-compat-breaking-старый-план-без-секции-feature5-feature6-feature7-feature8), [UC-6](USE_CASES.md#uc-6-edge-case-секция-в-неправильном-порядке-feature2)
...
Каноническое правило `.carl/rules/plan-pomogator/plan-pomogator.md` содержит четыре шага Two-Stage Workflow. Step 2 сначала определяет, содержится ли подтверждение в сильной директиве пользователя. Команды `делай план`, `пиши план`, `составляй`, `продолжай`, `go ahead` и `do it` после summary или ревью уже подтверждают scope: AI не задаёт повторный вопрос, а сразу пишет и валидирует план. Ожидание отдельного ответа допускается только при отсутствии такой директивы и реальной неоднозначности. Step 1 с human summary и Step 4 с ExitPlanMode остаются обязательными.
...
**Use Case:** [UC-4](USE_CASES.md#uc-4-backward-compat-breaking-старый-план-без-секции-feature5-feature6-feature7-feature8)
