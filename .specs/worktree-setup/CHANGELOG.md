...
- **Previously-skipped scenarios implemented** — bootstrap+registration (CORE024_04/05), ancestor-guard (CORE024_29), gh-auth pre-flight (CORE024_13), PR repo resolution via git remote (CORE024_11), session-pilot doctor contract (CORE024_16), and devcontainer bring-up (CORE024_31/32) now run as real integration tests using `writeMockInstaller` (mock `bin/cli.js`) + `makeMockBin` (PATH-shim `gh`/`docker`), no more `it.skip`. Helpers `makeMockBin`/`writeMockInstaller` added to `worktree-helpers.ts`; `makeTempGitRepo` now makes an initial commit (so `git worktree add` has a HEAD).
