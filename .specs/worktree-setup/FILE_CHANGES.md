...
| `tests/e2e/worktree-helpers.ts` | create | Integration harness: `makeTempGitRepo`, `makeTempDir`, `isolateHome`, `cleanupTempPaths`, `gitAvailable` — builds real temp git repos on the fly (no static fixtures, no installer) |
