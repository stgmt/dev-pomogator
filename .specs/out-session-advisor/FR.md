...
**Модель-консультация (опционально, модель-пара):** `consult.mjs` на ключевых точках
(done/recurring/plan) шлёт ограниченный пакет транскрипта (главный + субагенты + event-log)
сильной модели `ADVISOR_MODEL` (default `gpt-5.6-sol`) через `/v1/messages` и возвращает
совет «что перепроверить»; fail-open без env/транскрипта. Детерминированный verify остаётся
