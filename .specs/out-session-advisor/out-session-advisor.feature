...
Feature: OUTSESS001_session_advisor_and_parallel_safety
  ...
  (включая ход мысли субагентов), управляет воркером через ConPTY с ctl/rsp-протоколом, перепроверяет
  ...
  Background:
    ...
    And главный JSONL <main-session.jsonl> содержит события user/assistant с tool_use
...
# Часть A — Адвизор
  ...
  Scenario: OUTSESS001_01 живые субагенты видны в хвосте транскрипта
    ...
    Then в выводе присутствуют текстовые и tool-события файла subagents/agent-test.jsonl
  ...
  @feature2 @FR-2 @AC-2
  Scenario: OUTSESS001_06 промпт доставляется в ConPTY-воркер через ctl/rsp
    Given ConPTY-демон запущен с --dangerously-skip-permissions
    When адвизор пишет send в claude-ctl.json с utf8-промптом
    Then claude-rsp.json содержит out snapshot и pid воркера
    And промпт в поле ввода без искажения спецсимволов
