...
All stories are read-only. They require an explicit, versioned contract for the spec-generator-v4 MCP provider; they do not authorize edits to specifications, test results, graph data, Plane, or user credentials. Plane preview files are discovery references only until reuse, compatibility, routing, authentication, and licensing decisions are closed in the next phase.
