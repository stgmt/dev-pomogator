...
- This feature SHALL be a documented, pinned, and license-compliant vendored Plane source fork, not a screenshot-based reimplementation. Plane-derived files SHALL be retained only for board, UI, design-system, and runtime shell portions; Plane backend, domain, authentication, workspace, project, database, and external-service portions SHALL be replaced or bypassed.
