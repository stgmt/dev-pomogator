...
Если investigation подтвердит: hooks в `wt-manual-billing` ложатся в `settings.json` вместо `settings.local.json` (нарушение personal-pomogator FR-2) → починить в `src/installer/claude.ts` + integration test.
