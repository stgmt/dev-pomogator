...
> **Status: всё закрыто (2026-05-23).** Skill `.claude/skills/skills-rules-optimizer/` шипнут — `SKILL.md` + 9 scripts: `audit.ts` / `audit-rules.ts` / `audit-skills.ts` (token count, frontmatter validation, allowed-tools coverage, oversize cap), `detect-overlap.ts` (triple-axis Jaccard), `merge-skills.ts` (LLM-driven sub-agent synthesis), `verify-merge.ts` (ratchet/regression scorer), `check-antipatterns.ts`, `report.ts`, `shared.ts`. Phase 6 интеграция с `/suggest-rules`: `.claude/commands/suggest-rules.md` вызывает `audit.ts` после Phase 5. Тесты: `tests/e2e/sro-stubs.test.ts`. [FR-10](FR.md#fr-10-embedding-based-semantic-merge-out-of-scope) (embeddings) и [FR-11](FR.md#fr-11-auto-apply-без-human-review-out-of-scope) (auto-apply) explicitly OUT OF SCOPE. Audit-spec: 0 ERRORS / 4 WARNINGS (MISSING_AC stubs для OUT-OF-SCOPE FR-10/11 + DESIGN classification format).
