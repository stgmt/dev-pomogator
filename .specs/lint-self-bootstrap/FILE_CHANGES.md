...
| `tools/lint-self-bootstrap/run-lint.cjs` | create | Provide the builtins-only runtime self-bootstrap lint wrapper used by `npm run lint`, so a fresh checkout can prepare local eslint before running lint for [FR-1](FR.md), [FR-3](FR.md). |
