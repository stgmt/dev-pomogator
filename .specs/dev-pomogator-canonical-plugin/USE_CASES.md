...
- **EC-7 Hook runtime recovery without Node**: canonical cached-plugin hooks and dogfood source-tree hooks must begin with a portable shell preflight before invoking Node. If the expected runtime is absent or unusable, the preflight performs only the documented recovery/migration action and returns exit 0; it must not block Claude Code or run TypeScript directly in the shell.
