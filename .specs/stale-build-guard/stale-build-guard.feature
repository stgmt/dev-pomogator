Feature: GUARD001_Stale_Build_Guard
  ...
  # @feature1
  Scenario: GUARD001_02 deny when dist directory missing
  ...
  # @feature1
  Scenario: GUARD001_03 allow when build is fresh
    Given dist/index.js is newer than all src/**/*.ts files
    When build_guard hook receives a Bash command with "test_runner_wrapper --framework vitest"
    Then hook SHALL allow with exit code 0

  # @feature3
  Scenario: GUARD001_04 deny when Docker SKIP_BUILD is set
  ...
  # @feature3
  Scenario: GUARD001_05 deny when dotnet --no-build flag present
    Given the test command contains "--no-build" flag
    When build_guard hook receives a Bash command with "test_runner_wrapper --framework dotnet" and "--no-build"
    Then hook SHALL deny with exit code 2
    And deny message SHALL contain "Remove --no-build"

  # @feature3
  Scenario: GUARD001_06 allow for pytest without staleness check
    When build_guard hook receives a Bash command with "test_runner_wrapper --framework pytest"
    Then hook SHALL allow with exit code 0

  # @feature3
  Scenario: GUARD001_07 allow for go without staleness check
    When build_guard hook receives a Bash command with "test_runner_wrapper --framework go"
    Then hook SHALL allow with exit code 0

  # @feature3
  Scenario: GUARD001_08 allow for rust without staleness check
  ...
  # @feature1
  Scenario: GUARD001_10 allow for non-test commands (passthrough)
    When build_guard hook receives a Bash command with "ls -la"
    Then hook SHALL allow with exit code 0

  # @feature1
  Scenario: GUARD001_11 fail-open on invalid JSON input
    When build_guard hook receives invalid JSON on stdin
    Then hook SHALL allow with exit code 0

  # @feature1
  Scenario: GUARD001_12 fail-open on stat error
