...
## Second dogfood postmortem: harness identity, ownership, and evidence failure classes

### Provenance and status

This section records the second user-supplied dogfood incident from `E:\Note from ChatGPT.txt`. It is `[USER-SUPPLIED][UNVERIFIED_FOR_THIS_REPOSITORY]` and is not an authoritative local runtime trace. Adjacent-project commits, tests, model names, container names, and reported self-test counts are context only. No implementation, test, completion, or guarantee-tier claim follows from them.

### Failure classes converted into canonical design targets

1. **Root and isolation identity:** the packet/run declares expected repository root, exact existing worktree or explicit isolation, base SHA, owner task, and dirty-path allowlist. A mechanical normalized-root versus actual-git-top-level check runs before the first work action and blocks mismatch; an absolute prompt path cannot authorize it, and continuation cannot silently create a new worktree.
2. **Process-tree ownership and terminal stop:** each branch owns a Windows Job Object or Unix process group conceptually. Stop is terminal only after owner and all descendants/writers are zero, including PowerShell jobs, wrappers, WSL, nested CLIs, and child Claude processes. Terminal evidence contains `ownerStopped`, `descendantsRemaining`, and `writersRemaining`. Stop states are `PAUSED_RESUMABLE` and `TERMINATED_NO_RESUME`; contamination/context overflow requires a new worker with a recovery capsule.
3. **Single-writer run state and leases:** one CAS-based per-run state machine has exactly one mutating owner. Checkout-writer lock and external/shared-runtime lease are separate. APPLY is blocked until root verification, exclusive ownership, green preflight, and frozen baseline/plan. Nested fan-out requires a central ownership census.
4. **Worktree-safe external resources:** resource identity derives from run/worktree where possible. Labels carry repository, worktree, SHA, run owner, and lease. Reuse requires ownership and actual mount/source validation. Foreign resources are never deleted; failure preserves diagnostics.
5. **Typed captured process and terminal diagnostics:** argv arrays are the normal API, with separate UTF-8 stdout/stderr/evidence files, native exit code, atomic JSON, and failure diagnostics. Free-form script text is fallback only. A terminal error cannot be replaced by a warning or file-lock symptom.
6. **Transactional mutation and evidence pyramid:** baseline hashes are captured; writes use staged copies/quarantine; source replacement/commit waits for mandatory gates. Failed gates rollback or mark entries unproven. Typed collections distinguish original, staged, proven, rejected, deferred, and unproven-applied work. Completeness is ALL mandatory proof layers, not any green representative check.
7. **Probe truth and typed summaries:** fixtures/probes execute one canonical real API path and distinguish harness defect, capability gap, and product failure. Schema validation enforces typed summaries and `count == items.length`; external producer claims require independent real readback.
8. **Per-run journal and monitor lifecycle:** each run owns `state.json`, `progress.jsonl`, `commands/`, `artifacts/`, and `terminal.json`; events include runId, monotonic seq, owner task/PID, worktree, phase, and status. Monitors filter runId/seq, inherit owner, terminate with owner, and old pulses/stale monitors never prove current progress.
9. **Bounded recovery and context:** recovery capsules are 1–3 KiB with root, owner, base SHA, dirty paths, accepted evidence/commits, unproven work, last green gate, blocker, next action, and do-not-touch. Lazy references replace full transcript replay. Two repeated infrastructure failures enter HARNESS_REPAIR and block domain apply. User updates are phase start, new confirmed blocker, and terminal result rather than pulse spam.
10. **Observability and verdict:** status correlates agent notifications, per-run journal, detached descendants, diagnostics, dirty worktree, lease, proof-layer completeness, and terminal marker. Global BDD/log green cannot green an active incomplete run. Productive time, recovery time, restarts, stale writers, false blockers, and context overflows are exposed when available.
11. **Fundamental identity invariant:** Agent/Workflow branch → repository root/worktree → process group → shared-resource lease → runId → proof phase is one atomic binding. Any broken link blocks mutation and completion.

### Authoritative replay boundary

The original run-state, per-run journals, process-group scans, terminal diagnostics, lease/resource labels and mount/source evidence, dirty-worktree/base-SHA evidence, and independent producer readback were not supplied as local artifacts in this authoring operation. Therefore authoritative replay remains `REPLAY_UNAVAILABLE`. A future replay fixture SHALL preserve the supplied failure classes without inventing evidence, and every adjacent-project claim SHALL remain context-only until independently proven.
