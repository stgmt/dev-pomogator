...
- [ ] Создать тестовый .feature `tests/e2e/architecture-decision/architecture-decision-builder.feature` (копия spec .feature, @feature1..@feature10) -- @feature1 — id: t01 — Status: TODO | Est: 20m
...
- [ ] Создать fixture `tests/fixtures/architecture-decision/greenfield-prd.md` — PRD без build-manifest, lifecycle: shared -- @feature1 — id: t02 — Status: TODO | Est: 15m
  _Source: DESIGN.md "BDD Test Infrastructure" > "Test Data & Fixtures"_
  **Done When:**
  - [ ] PRD ~80 строк, нет упоминаний стека/build-manifest
- [ ] Создать fixture `tests/fixtures/architecture-decision/brownfield-prd.md` — PRD с упоминанием стека для hard-OUT, lifecycle: shared -- @feature1 — id: t03 — Status: TODO | Est: 10m
  _Source: DESIGN.md "BDD Test Infrastructure" > "Test Data & Fixtures"_
  **Done When:**
  - [ ] PRD упоминает существующий Django+Postgres стек
- [ ] Создать fixture `tests/fixtures/architecture-decision/expected-axes.json` — golden output axis-detector, lifecycle: shared -- @feature1 — id: t04 — Status: TODO | Est: 10m
  _Source: DESIGN.md "BDD Test Infrastructure" > "Test Data & Fixtures"_
  **Done When:**
  - [ ] JSON с ожидаемыми AxisCandidate для greenfield-prd
- [ ] Создать cleanup hook (inline afterEach в test files) — удаляет temp output dir, scope: per-test -- @feature2 — id: t05 — Status: TODO | Est: 15m
...
- [ ] Создать 5 test files (ARCH001-005, заглушки с failing assertions) -- @feature1 — id: t06 — Status: TODO | Est: 30m
  **Done When:**
...
- [ ] Реализовать `axis-detector.ts` (detectAxes 3-layer + hard-OUT brownfield) -- @feature1 — id: t07 — Status: TODO | Est: 45m
...
- [ ] Реализовать `escape-log.ts` (JSONL append-only) -- @feature10 — id: t08 — Status: TODO | Est: 20m
...
- [ ] Реализовать `live-fetch.ts` (версии + 24h cache) -- @feature8 — id: t09 — Status: TODO | Est: 30m
  _Requirements: [FR-8](FR.md#fr-8-anti-bias-guardrails)_
  **Done When:**
  - [ ] версия кешируется 24h в .architecture-cache.json
  - [ ] mock WebFetch в тесте
- [ ] Реализовать `artefact-generator.ts` (Fisher-Yates seeded + word-budget) -- @feature2 — id: t10 — Status: TODO | Est: 45m
...
- [ ] Реализовать `html-renderer.ts` (self-contained, inline CSS, mermaid gated) -- @feature2 — id: t11 — Status: TODO | Est: 45m
...
- [ ] Реализовать `open-in-browser.ts` (cross-platform, ENOENT-safe) -- @feature3 — id: t12 — Status: TODO | Est: 30m
...
- [ ] Реализовать `index-compiler.ts` (idempotent AUTOGEN markers) -- @feature5 — id: t13 — Status: TODO | Est: 30m
...
- [ ] Реализовать `audit.ts` (ARCHITECTURE_COVERAGE) -- @feature9 — id: t14 — Status: TODO | Est: 30m
...
- [ ] Реализовать `architecture-decision-cli.ts` (5 команд dispatcher) -- @feature4 — id: t15 — Status: TODO | Est: 45m
...
- [ ] Verify: сценарии @feature1,2,3,5,8,9,10 переходят из Red в Green — id: t16 — Status: TODO | Est: 15m
  **Done When:**
  - [ ] `/run-tests architecture` — ARCH001-004 green

## Phase 2: Skill + rules + create-spec integration (Green)

- [ ] Создать SKILL.md + 3 references (axis-catalog/variant-format-spec/html-style-guide) -- @feature7 — id: t17 — Status: TODO | Est: 45m
...
- [ ] Создать 2 rules (when-to-build + escape-hatch-audit) -- @feature1 — id: t18 — Status: TODO | Est: 30m
...
- [ ] Создать 2 templates (ARCHITECTURE_AXIS + ARCHITECTURE_INDEX) -- @feature2 — id: t19 — Status: TODO | Est: 20m
...
- [ ] Интеграция create-spec Phase 1.75 (phase1.75 ref + SKILL.md nav + phase1.5 edit) -- @feature7 — id: t20 — Status: TODO | Est: 45m
  _Requirements: [FR-7](FR.md#fr-7-два-режима-запуска-standalone-create-spec-phase-175)_
...
- [ ] Добавить ARCHITECTURE_COVERAGE 9-ю audit категорию (overview edit + audit-architecture-coverage ref) -- @feature9 — id: t21 — Status: TODO | Est: 20m
  _Requirements: [FR-9](FR.md#fr-9-audit-category-architecturecoverage)_
  _Leverage: references/phase3plus_audit-variant-coverage.md_
  **Done When:**
  - [ ] overview содержит 9-ю категорию
  - [ ] @feature9 scenario passes
- [ ] Обновить extension.json (skills/skillFiles/ruleFiles/toolFiles, bump 1.21.0) -- @feature7 — id: t22 — Status: TODO | Est: 30m
  _Requirements: [FR-7](FR.md#fr-7-два-режима-запуска-standalone-create-spec-phase-175)_
  _Config: см. FILE_CHANGES.md_
  **Done When:**
  - [ ] все новые файлы перечислены в манифесте
  - [ ] extension-layout-validate.ts exit 0
- [ ] Реализовать cli-integration end-to-end + Phase 1.75 + audit (ARCH005) -- @feature4 — id: t23 — Status: TODO | Est: 45m
...
- [ ] Verify: сценарии @feature4,6,7,9 переходят из Red в Green — id: t24 — Status: TODO | Est: 15m
  **Done When:**
  - [ ] `/run-tests architecture` — ARCH005 green
- [ ] Создать evals/evals.json (8 deterministic cases) + rubric.json (R1-R9) -- @feature11 — id: t25 — Status: TODO | Est: 30m
...
- [ ] Создать golden bench scenario-bhph (copy ARCHITECTURE_PROPOSAL.md) + scenario-saas/cli-tool -- @feature11 — id: t26 — Status: TODO | Est: 20m
  _Requirements: [FR-11](FR.md#fr-11-eval-suite-debug-benchmark-качества-2-слоя)_
...
- [ ] Реализовать eval-suite.test.ts (ARCH006) — deterministic runner + rubric grading -- @feature11 — id: t27 — Status: TODO | Est: 45m
  _Requirements: [FR-11](FR.md#fr-11-eval-suite-debug-benchmark-качества-2-слоя)_
  **Done When:**
...
- [ ] Обновить CLAUDE.md (2 новых rule в таблицу Rules) -- @feature1 — id: t28 — Status: TODO | Est: 10m
  _Requirements: [FR-1](FR.md#fr-1-axis-enumeration-из-prd)_
  _Config: claude-md-glossary rule_
  **Done When:**
  - [ ] when-to-build-architecture + escape-hatch-audit в таблице
- [ ] Рефакторинг после прохождения всех сценариев — id: t29 — Status: TODO | Est: 30m
  **Done When:**
  - [ ] Все BDD сценарии @feature1..@feature10 GREEN
  - [ ] Code duplication score (jscpd) ≤ baseline
- [ ] Final verification — id: t30 — Status: TODO | Est: 15m
  **Done When:**
...
- [x] FR-13 synthesis.ts + CLI `synthesis` + SYNTHESIS.md.template -- @feature14 — id: t31 — Status: DONE | Est: 45m
...
- [x] FR-16 policy renderer (pickRecommended + demonstration table + badge) -- @feature17 — id: t32 — Status: DONE | Est: 45m
...
- [x] FR-14 correction_log render + FR-15 live-context7 дисциплины -- @feature15 — id: t33 — Status: DONE | Est: 30m
...
- [x] SKILL.md workflow (policy step 1.5 + synthesis step 3.5 + context7) -- @feature16 — id: t34 — Status: DONE | Est: 30m
...
- [x] eval-10/11 + rubric R21-R23 + extension.json bump 1.22.0 -- @feature17 — id: t35 — Status: DONE | Est: 30m
...
- [x] FR-17 две линзы + scorecard карта-сравнение + reality-check (R24, BLOCKING) -- @feature20 — id: t36 — Status: DONE | Est: 45m
...
- [x] FR-18 экономика: cost_at_scale + time_costs + exit_cost + door_type + sensitivity (R25, BLOCKING) -- @feature21 — id: t37 — Status: DONE | Est: 45m
...
- [x] Completeness-сценарии coverage (другая сессия 84f3042e, FR-12) -- @feature12 @feature13 @feature18 @feature19 — id: t38 — Status: DONE | Est: 5m
...
- [x] FR-19 full-report: единый ARCHITECTURE.html через renderers + AXIS-model.json персист -- @feature22 — id: t39 — Status: DONE | Est: 60m
...
- [x] FR-20 verify-log + audit-markers + detect confidence/domain-gate + validateAxisModel + cost-disclaimer -- @feature23 — id: t40 — Status: DONE | Est: 90m
...
- [x] FR-21 architecture-gate — гарантированный Phase 1.75 (PreToolUse hook) -- @feature24 — id: t41 — Status: DONE | Est: 45m
