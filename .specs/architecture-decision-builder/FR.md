...
## FR-19: Единый self-contained ARCHITECTURE.html (full-report)

Команда `full-report <spec-dir> [insights.json]` собирает ОДИН self-contained `ARCHITECTURE.html` из персистентных `AXIS-*.model.json` (пишутся `generate-axis` автоматически рядом с md/html), опциональных cross-axis insights и `COMPLETENESS.md` ledger. Структура: H1 + index status-матрица (якорные ссылки на оси) + по каждой оси полная секция (`renderAxisSection` — богатые карточки: 2 линзы R24 + scorecard + reality + экономика R25) + cross-axis synthesis секция + completeness таблица. Рендерится через ТЕ ЖЕ функции что per-axis страницы (`renderAxisSection`/`renderSynthesisSection`), НЕ скрейпом HTML — отчёт наследует весь rich-контент даром. Self-contained (inline CSS, без внешних `<link>`). Запускается ПОСЛЕ всех `generate-axis` + `synthesis` (финал workflow). INDEX.html остаётся лёгкой статус-матрицей; ARCHITECTURE.html — «всё в одном».

**Связанные AC:** [AC-19](ACCEPTANCE_CRITERIA.md#ac-19-fr-19)
**Use Case:** [UC-1](USE_CASES.md#uc-1-standalone-выбор-стека-для-greenfield-проекта-happy-path)

## FR-20: Anti-hallucination integrity — verify-log + unbacked-marker guard

Маркер `[VERIFIED via context7:<lib> <ver>]` ОБЯЗАН соответствовать РЕАЛЬНОМУ context7-вызову, иначе это фабрикованная цитата (та галлюцинация, которую R3/R15 призваны предотвращать — presence маркера ≠ правдивость). Команда `record-verify <spec-dir> <lib> [ver]` пишет реальную верификацию в `.architecture-verify.jsonl` (append-only). Команда `audit-markers <spec-dir>` сканит `AXIS-*.md` на context7-VERIFIED маркеры, сверяет с verify-log; маркер с lib без backing-записи → `UNBACKED_VERIFIED_MARKER` severity WARNING; все backed + ≥1 маркер → `MARKERS_BACKED` INFO. Делает дисциплину аудируемой (как scope-gate escape-log), а не trust-based. Сопутствующая интегрити-жёсткость: detect-axes confidence/domain-gating (Networking/Hardware-оси требуют ≥2 сигналов — убирает keyword over-match), `validateAxisModel` fail-clear (битая модель → внятная ошибка с именем поля, не opaque crash), cost-числа помечаются «оценка · knowledge-cutoff».

**Связанные AC:** [AC-20](ACCEPTANCE_CRITERIA.md#ac-20-fr-20)
**Use Case:** [UC-1](USE_CASES.md#uc-1-standalone-выбор-стека-для-greenfield-проекта-happy-path)

## FR-21: Гарантированный Phase 1.75 — architecture-gate (PreToolUse, не trust-based)

Phase 1.75 был workflow-инструкцией в create-spec SKILL.md (агент «должен» вызвать skill) — без механического принуждения агент мог проскочить и писать FR/DESIGN в стек-вакууме. `architecture-gate.ts` (PreToolUse Write|Edit hook) делает фазу **гарантированной**: при записи Requirements-артефакта (`FR.md`/`DESIGN.md`/`REQUIREMENTS.md`/`NFR.md`/`ACCEPTANCE_CRITERIA.md`/`FILE_CHANGES.md`) для **greenfield** спеки (detect-axes на PRD-эквиваленте USER_STORIES+USE_CASES+RESEARCH+PRD вернул axes>0) hook ТРЕБУЕТ `.specs/{slug}/ARCHITECTURE/` с артефактами (INDEX/COMPLETENESS/AXIS-*) ИЛИ явный skip (`[skip-architecture-axis: <reason>]` в RESEARCH, либо `ARCHITECTURE/.skip` файл). Иначе `permissionDecision: deny` с actionable-сообщением (run skill / skip). Discovery/Context-файлы НЕ гейтятся. Migration guard: `.progress.json version < 4` → no-op. Новые спеки штампуются `version: 4` (`createDefaultProgressState` в `specs-generator-core.mjs`) — именно это заставляет gate срабатывать на реальных свежесозданных спеках; pre-architecture v1–v3 grandfathered (gate no-op, не блокирует редактирование их FR). Form-guards остаются на floor `>= 3` (`PROGRESS_SCHEMA_VERSION`). Fail-open: ошибка hook → allow. Тот же класс enforcement что phase-gate.ts — закрывает «trust-based» дыру (как FR-20 закрыл fabricated-маркеры).
