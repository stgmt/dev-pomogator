...
Installer уже собирает все paths в `managedByExtension` map во время `installClaude()`. `addProjectPaths()` сохраняет в config. Вся информация для gitignore генерации уже есть — нужен только writer (~~`src/installer/gitignore.ts`~~ (removed in v2 migration)).
...
~~`src/installer/memory.ts:610-675`~~ (removed in v2 — no canonical replacement) — `registerClaudeMemMcp()` пишет только в `~/.claude.json` (`path.join(os.homedir(), '.claude.json')`). Это правильное поведение, spec фиксирует как invariant.
...
| `installedHooksByExtension` | ~~`src/installer/claude.ts:373`~~ (removed in v2 — no canonical replacement) | Per-extension hook commands list | Authoritative source для hook identification в migration |
...
| `installClaude` step 9 `addProjectPaths` | ~~`src/installer/claude.ts:270`~~ (removed in v2 — no canonical replacement) | Persists managed data | Wire-point для gitignore writer |
| `installExtensionHooks` | ~~`src/installer/claude.ts:371-544`~~ (removed in v2 — no canonical replacement) | Current hook merge logic | Extends для settings.local.json routing |
...
| `getExtensionSkills` | ~~`src/installer/claude.ts:173-195`~~ (removed in v2 — no canonical replacement) | Skills install flow | Reuse для personal-pomogator extension |
