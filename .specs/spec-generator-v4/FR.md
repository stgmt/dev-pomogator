...
FR-44 ловит обратные дыры эвристикой по тексту (FR без ресерча, upstream-unlinked). FR-47 делает паутину НАСТОЯЩЕЙ: дизайн-решения, истории и ресерч-находки моделируются узлами графа с реальными рёбрами к требованию (не текст-скан тела — «костыль», по требованию owner'а). Цель: у любой фичи 100% = все ноги привязаны (критерии + сценарий + задача + ресерч + дизайн), enforced в двери + видно в трассе в обе стороны.
...
- **FR-47b (вердикт полноты требования):** conformance SHALL давать `FR_NO_DESIGN` (FR без покрывающего Decision), зеркально `FR_NO_RESEARCH`; и единый вердикт «полнота требования» — FR без хоть одной ноги (AC / сценарий / задача / ресерч / дизайн) подсвечивается. Поэтапно detect→retrofit→gate (как FR-46c), дельта-скоуп — не клинить дверь на предсуществующих.
