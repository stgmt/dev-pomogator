...
**FR-7c (wiki-link form — adopt the form Marksman resolves):** Specs currently use markdown links (`[AC-1.1](#ac-1-1)`), not live `[[wiki-links]]`. Marksman matches by full heading slug. **CONFIRMED end-to-end (2026-06-04)** via a real `claude -p` session using the native `LSP` tool: a short heading `## Note` (slug `note`) makes `[[Note]]` resolve to every occurrence (`documentSymbol` listed the heading; `references` returned the def line + both `[[Note]]` link lines — matching ground-truth exactly), whereas `## FR-1: Title` (slug `fr-1-title`) does NOT resolve `[[FR-1]]`. Specs SHALL therefore adopt ONLY the resolving form — short ID-only headings (`## FR-1` → slug `fr-1` → `[[FR-1]]` works). Migrating specs to a non-resolving form is forbidden — it repeats «installed ≠ integrated» ([[dead-integration-guard]]). The spec-migration itself (rewriting headings/links) is a separate follow-up.
