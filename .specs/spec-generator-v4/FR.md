...
**FR-7a (LSP bridge — the runtime consumer):** The MCP server SHALL resolve the Marksman binary **package-first** (`resolve-binary.ts`): (1) a system-package install on `PATH` (`winget`/`brew`/`snap`/`nix`) — preferred, no download, no pinned hashes; (2) else a managed download in `.dev-pomogator/bin/` (integrity-checked against `marksman-hashes.json`, whose sha256 are COMPUTED by `cli-update-hashes.ts`, never hand-pasted); (3) else none → js-fallback. When a binary resolves, the server SHALL spawn it (`marksman server`), complete the `initialize`/`initialized` handshake over JSON-RPC/stdio, and expose `definition`/`references`. The handshake MUST declare `capabilities.workspace.workspaceFolders` and the workspace MUST carry a project-root marker (`.marksman.toml`/`.git`) — otherwise Marksman rejects the folder («Workspace folder is bogus») and returns empty refs (verified by capture, Linux + Windows). Bridge startup failure SHALL degrade to the graph fallback without crashing (fail-open); `shutdown`/`exit` on server stop.

**FR-7a-trigger:** Because dev-pomogator ships as a marketplace plugin (no `npm install` in the user env, so `package.json` postinstall never fires), a `SessionStart` hook (`ensure-marksman.ts`) SHALL run the resolver and, when no binary is present, fire a DETACHED background managed download (with back-off after a failed attempt) so a later session resolves it. The hook MUST NOT block session start.
...
**FR-7c (division of labour — Marksman-first for primitives, graph for spec-domain):** Navigation AND editing primitives over wiki-links — `definition`, `references`, **`rename`** (rename FR-1 → update every `[[FR-1]]`), `completion`, `hover`, code-actions, file-rename→link-fix — SHALL be served by Marksman's mature, battle-tested tools, NOT reimplemented in custom code. The custom graph SHALL retain ONLY what Marksman has no concept of: spec-domain traceability (FR→AC→Scenario→Task→test coverage via `get_trace`/`get_coverage`), the honesty-gate (FR-32), and conformance. The graph's `wikilinks.ts` resolver is the FALLBACK when Marksman is absent (AC-3.3 in js-fallback mode), not the primary. The system supports both READING (agent queries) and EDITING (refactoring specs) — editing leans on Marksman's `rename`/code-actions, after which the graph re-derives traceability.

**FR-7c-prereq (verify the resolvable link form BEFORE migrating specs):** Specs use markdown links (`[AC-1.1](#ac-1-1)`), not live `[[wiki-links]]`. Before migrating them to wiki-links, the EXACT `[[…]]` form Marksman resolves on real ID-prefixed headings (`## FR-1: Title`) MUST be empirically confirmed — via Marksman `completion` (it enumerates the targets it knows) and `definition` queried at the LINK position — because experiments at the heading position were inconclusive. Migrating specs to a form Marksman cannot resolve repeats the «installed ≠ integrated» failure class ([[dead-integration-guard]]).
