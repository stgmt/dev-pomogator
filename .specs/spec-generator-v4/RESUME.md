...
**P17-6 live-verification ПРОГНАНА (2026-06-08) — enforce НЕ готов, гейт честно сказал «рано».**
Сначала всплыл КРИТ: MCP-бандл был МЁРТВ headless — `server.bundle.mjs` печатал `usage: check.mjs`
и выходил (anchor-integrity/check.mjs direct-run guard срабатывал в бандле; внесено P17-2, у юзеров
дверь не открывалась). Починено (basename-guard, be9a1cb), бандл отдаёт 19 тулзов; `hooks-stdin-e2e`
переведён на БАНДЛ + список 13→19 (был RED с P17-1/2 — я зря списал в «флейки»). ЗАТЕМ live
`claude -p` на естественной спек-задаче: агент прошёл МИМО двери — 2 shadow-violations
(`Grep`+`Read` по `.specs/FR.md`), т.к. (1) MCP-тулзы не выданы headless-сессии («permission I don't
have granted»), (2) голая задача не триггерит мигрированный скилл. **Вывод: флипать enforce СЕЙЧАС
нельзя** — агента задеяли бы без MCP-fallback. Предусловия P17-6 до флипа: (а) выдать spec-MCP-тулзы
в permissions проекта; (б) гарантировать что агент предпочитает дверь (скилл-триггер ИЛИ enforce+выданный
MCP); (в) повторный live с выданным MCP → 0 residual → ТОЛЬКО ТОГДА флип. enforce остаётся OFF.
