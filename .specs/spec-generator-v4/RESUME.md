...
поведение, агент идёт прямо в дверь. **Остаток до ПОСТОЯННОГО флипа:** (а) грант `mcp__dev-pomogator-specs__*`
в `.claude/settings.json` permissions — ДЕЙСТВИЕ ЮЗЕРА (classifier не даёт агенту расширять свои права);
(б) прогнать ПОД enforce ещё и WRITE-путь (create-spec через apply_spec_change), не только read; (в) тогда
флип `SPEC_ACCESS_ENFORCE=true` в settings. enforce в репо остаётся OFF (доказано только per-session env).
