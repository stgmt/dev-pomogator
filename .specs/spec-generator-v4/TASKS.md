...
- [ ] P19-6: MCP-дверь к ПОДКАТАЛОГАМ спеки (ARCHITECTURE/ attachments/ .architecture-research/) — id: p19-subdir-door — Status: IN_PROGRESS (read-side DONE; consumer-миграция + write-resolution остаток) | Est: 300m
  ...
  **Done When:**
  ...
  - [ ] **WRITE-side ОСТАТОК (коррекция 2026-06-08 — моё прежнее «всё покрыто carve-out» было НЕПОЛНЫМ):** arch-research-workflow ЗАПОЛНЯЕТ 7 stage-файлов `.architecture-research/<N>-stage.md` АГЕНТОМ через Write-тул (freeform-проза, не CLI) — под enforce spec-access-guard блокирует Write в `.specs/**`, а carve-out only для Bash engine-CLI, НЕ для Write-тула. Решение (consistent с Option A «extend door»): расширить apply_spec_change/validateTarget/writeDocAtomic на subpath (containment через `resolveSpecDoc`; writeDocAtomic mkdir parent) + для non-graph subdir-доков (.architecture-research/) ПРОПУСКАТЬ graph-гейты form/anchor/conformance (они для top-level FR/AC/TASKS/feature, не для research-прозы) ЛИБО stage-write через CLI scripts/write-stage.ts (carve-out). Реализовать следующим ходом
