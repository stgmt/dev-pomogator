...
- **FR-26 NEW** — Per-session rows (1 row per JSONL UUID, not per cwd). Motivating diagnostic 2026-05-13: user had 3 LIVE sessions в `D--repos-dev-pomogator/` (own session #1 + own session #2 + assistant's EnterWorktree-induced session) — все with distinct UUIDs, but dashboard collapsed all to 1 row showing only newest mtime. Fix: indexer emits row per JSONL file. Schema additions: `session_uuid` field per row. Per-cwd computed values (repo/branch/git status / orphan flag / window-open flag) cached per cwd and attached to each row sharing that cwd. Resume button uses **row-specific UUID** (not newest). Git-only worktrees (Source C from FR-24) emit row with `session_uuid: null` + Resume disabled. Implementation pending T54.
- **FR-25 NEW** — Process-based "open window" indicator (отдельный signal от JSONL mtime LIVE). Motivating UX: user keeps 4 Claude Code windows open для часов — only 1 is actively typing, остальные 3 ждут input. JSONL mtime stays stale в idle windows → dashboard misleadingly shows "idle Xh" для open windows. Fix: indexer scans `claude.exe` processes per-OS (Win32_Process + parent chain analysis on Windows / `/proc/<pid>/cwd` on Linux / `lsof` on macOS), extracts cwds, sets `claude_window_open: bool` + `claude_window_pids: list[int]` per row. Frontend 3-state Status column: `🟢 LIVE` (writing now) > `💡 Open` (window exists, idle) > `idle Xs`. Cache TTL 5s + fail-open under NFR-Perf 100ms budget. Excludes Claude.ai desktop app (cwd == install dir) from row emission. Implementation pending T52-T53.
- **FR-24 NEW + KD-12** — Sessions + worktrees UNION (НЕ замена worktree list). Discovered via real-host diagnostic 2026-05-13: out of 7 project dirs в `~/.claude/projects/`, 2 были non-git cwds (`C--Users-stigm-Desktop`, `D--repos`) — fully resumable, но **invisible в worktree-centric dashboard**. v0.4 indexer теперь emit-ит **три источника** rows: (A) `~/.claude/projects/*` где decoded cwd = registered git worktree (full git fields + sessions, как раньше); (B) `~/.claude/projects/*` где decoded cwd НЕ git (orphan: is_orphan=true, empty repo/branch); (C) `git worktree list` worktrees где Claude никогда не запускался (full git fields + claude_max_mtime=null). ВСЕ git worktrees из configured repos продолжают появляться (Source A + Source C). Schema additions: `is_orphan: bool`, `is_stale: bool`. Dedup A∩C → A wins. Implementation pending T49-T51.
...
**Имплементация v0.4 — shipped 2026-05-13** в этом же PR:

- `indexer.py` (+170 LOC): `build_session_index()` + cached + `_claude_jsonls_lightweight()` + `_decode_claude_dir_name()` helpers
- `process_scanner.py` (NEW, 200 LOC): per-OS process scan (Windows Win32_Process parent chain / Linux /proc/cwd / macOS lsof) с 5s cache + opt-out env var + fail-open
- `handlers.py`: `/api/index` → new builder, health version 0.4.0 + platform field
- `server.py`: re-export new indexer symbols
- `frontend.py` (+120 LOC inline JS): cache key v3→v4 + one-shot purge, 3-state Status formatter (LIVE > Open > idle), Resume uses row.session_uuid, orphan/stale visual rendering
- `tests/test_per_session_index.py` (NEW, 11 tests passing): FR-26 per-session expansion + FR-24 orphan/Source C/dedup/meta-filter + FR-25 process scan fail-open paths

Verified on live Windows 11 host 2026-05-13: 32 rows total, per-session granularity confirms 3 distinct UUIDs in dev-pomogator (previously collapsed to 1 row), 2 orphan rows for non-git cwds (Desktop, D:\repos), process scanner detects open windows in cwds where JSONL stale.
