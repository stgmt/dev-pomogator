...
> **Status: всё закрыто (2026-05-23).** Реализация в `tools/specs-generator/variant-matrix/` (5 файлов: trigger-phrases.ts mechanical regex EN+RU + 14 axis nouns, parsers.ts + escape-log.ts + audit.ts VARIANT_COVERAGE category + variant-matrix-cli.ts). Skill `.claude/skills/variant-matrix-build/SKILL.md` интегрирован в create-spec Phase 2 step 4c (см. `.claude/skills/create-spec/references/phase2_requirements-and-design.md`). Audit category VARIANT_COVERAGE блокирует STOP #3 на polymorphic FRs без enumerated matrix. Audit log JSONL `.claude/logs/spec-variant-matrix-escapes.jsonl` для `[skip-variant-matrix: ...]` escapes. 19 тестов в `tests/e2e/specs-generator-variant-matrix.test.ts` зелёные. Audit-spec: 0 ERRORS / 5 WARNINGS (cosmetic — MISSING_AC для FR-5/8/9 ([FR-9](FR.md#fr-9-pretooluse-form-guard-на-writeedit-frmd-out-of-scope) OUT OF SCOPE) + TEST_DATA_ACTIVE classification format + meta-recursive VARIANT_COVERAGE на FR-5).
