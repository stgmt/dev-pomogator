...
- **FR-23** (DONE): задачи со статусом `BLOCKED` и спеки, явно помеченные `backlog` (parked), НЕ считаются «actionable open work» и НЕ взводят гейт. Заблокированную задачу агент физически не может сделать (нужен внешний разблок — напр. пароль владельца для Docker), припаркованная спека — отложена. Гнать агента «доделать» их — чрезмерная строгость (инцидент 2026-06-25: docker-sudo петля — судья 4× подряд блокировал честный стоп, требуя сделать BDD-прогон, который упирается в WSL-`sudo`, доступный только владельцу). Реализация: `computeTaskCensus` (`blocked` исключён из `open`) + `liveOpenForUncensusedSlugs` (skip BLOCKED-строк + `readSpecStatus()==='backlog'`). Тест: CEGATE001_42. Доказано: openWork сессии 4→0 после исключения BLOCKED Phase-2 и backlog'а WIP-спеки.
