...
**Связанные AC:** [AC-1](ACCEPTANCE_CRITERIA.md#ac-1-fr-1-4-tab-tui-interface-feature1)
...
**Связанные AC:** [AC-2](ACCEPTANCE_CRITERIA.md#ac-2-fr-2-test-tree-view-feature2)
...
**Связанные AC:** [AC-3](ACCEPTANCE_CRITERIA.md#ac-3-fr-3-real-time-log-viewer-feature3)
...
**Связанные AC:** [AC-4](ACCEPTANCE_CRITERIA.md#ac-4-fr-4-monitoring-dashboard-feature4)
...
**Связанные AC:** [AC-5](ACCEPTANCE_CRITERIA.md#ac-5-fr-5-failure-analysis-feature5)
...
**Связанные AC:** [AC-6](ACCEPTANCE_CRITERIA.md#ac-6-fr-6-yaml-v2-protocol-feature6)
...
**Связанные AC:** [AC-7](ACCEPTANCE_CRITERIA.md#ac-7-fr-7-universal-framework-adapters-feature6)
...
**Связанные AC:** [AC-8](ACCEPTANCE_CRITERIA.md#ac-8-fr-8-yaml-polling-feature1)
**Use Case:** [UC-1](USE_CASES.md#uc-1-запуск-tui-и-мониторинг-тестов-feature1)
...
**Связанные AC:** [AC-10](ACCEPTANCE_CRITERIA.md#ac-10-fr-10-sessionstart-hook-feature7)
**Use Case:** [UC-7](USE_CASES.md#uc-7-сосуществование-с-test-statusline-feature7)
...
**Связанные AC:** [AC-13](ACCEPTANCE_CRITERIA.md#ac-13-fr-13-rule-centralized-test-runner-feature13)

## FR-15: Build Guard Hook @feature15

PreToolUse hook (`tools/tui-test-runner/build_guard.ts`) SHALL block test commands when the build is stale or build safety flags are misused. The hook SHALL:

1. Be registered in the plugin hooks registry (`.claude-plugin/hooks.json`) as a Bash PreToolUse entry, ordered BEFORE `test_guard`.
2. Deny (exit 2) when the vitest/jest cwd has a `src/` directory but no `dist/` directory — build artifacts are missing.
3. Deny (exit 2) when a Docker test command is run with `SKIP_BUILD=1` env — Docker build must not be skipped.
4. Deny (exit 2) when a dotnet test command carries the `--no-build` flag — stale binaries risk.
5. Allow (exit 0) for interpreted frameworks (pytest, go, rust) without a staleness check.
6. Allow (exit 0) when `SKIP_BUILD_CHECK=1` env is set — explicit bypass.
