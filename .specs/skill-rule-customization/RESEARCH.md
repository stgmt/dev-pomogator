...
Прецедент 1:1: `src/updater/hook-migration.ts` крутится **на каждом Stop-событии** (до cooldown) и мигрирует устаревший формат «даже у проектов, которые никогда не апдейтятся» (`hook-migration.ts:7-8`).
