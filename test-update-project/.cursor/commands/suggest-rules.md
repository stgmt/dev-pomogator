...
Отфильтровать entries с `status === "pending"`. Если 0 pending → вывести пустое сообщение → перейти к Phase 0.
...
**После Phase 5 (file creation)**, для entries, конвертированных в rules:
- `status = "consumed"`
...
**Выполнить:** `Glob(".cursor/rules/*.mdc")`
...
├── my-pattern.mdc
├── api-guidelines.mdc
├── no-hardcoded-values.mdc
└── ...
```

**ВАЖНО:** Правила хранятся как плоские `.mdc` файлы напрямую в `.cursor/rules/`, БЕЗ подпапок.
...
**Источники доменов:**
1. Имена существующих rules (префиксы `zoho-*`, `postgres-*`, `docker-*`, ...).
...
| ⚡ **Rule Violations** (T4) | Claude нарушил существующее правило из `.cursor/rules/` |
...
1. 🔴 no-direct-prod-db-edits.mdc
   Суть: "Никогда не редактируй production базу напрямую"

2. 📋 new-model-pricing-checklist.mdc
   Суть: "При добавлении модели: проверь ratio, tier, limits"
```

### Категории извлечения

| Категория | Формат имени файла | Когда использовать |
|-----------|-------------------|-------------------|
| 🔴 **Antipattern** | `no-<что-нельзя>.mdc` | Что НЕ делать (плохой подход из сессии) |
| 🟢 **Pattern** | `<паттерн-название>.mdc` | Как делать правильно |
| 📋 **Checklist** | `<задача>-checklist.mdc` | Список проверок для повторяющихся задач |
| ⚠️ **Gotcha** | `<технология>-gotcha.mdc` | Подводные камни, неочевидные нюансы |
| 🎯 **Skill** | `<workflow-название>.mdc` | Multi-step workflow с reusable assets |
| 🪝 **Hook** | `<hook-название>.mdc` | Автоматическая проверка/действие на IDE event |

**ВСЕ правила создаются в `.cursor/rules/<name>.mdc` — БЕЗ подпапок!**
...
**REUSABILITY подсказка:** domain-specific паттерн = полный балл (30), если домен есть в `Project domains` из Phase 0.5. `.cursor/rules/` — это именно место для проектных знаний, domain-specific НЕ штрафуется.
...
1. Проверить существующие skills/workflow-файлы в проекте
2. Сравнить trigger phrases и workflow steps
3. Если совпадение >80% → DUP, 30-80% → MERGE (добавить reference/step)

**Для hook-кандидатов:**
1. Проверить существующие hooks в конфигурации проекта
...
- **Путь:** `.cursor/rules/no-direct-prod-db-edits.mdc`
...
> **Принципы:** target ~1500-2000 слов. Императивная форма (глагол первым).
...
**ВСЕ правила создаются в плоской структуре:**

| Тип | Путь |
|-----|------|
| 🔴 antipattern | `.cursor/rules/no-<что-нельзя>.mdc` |
| 🟢 pattern | `.cursor/rules/<паттерн>.mdc` |
| 📋 checklist | `.cursor/rules/<задача>-checklist.mdc` |
| ⚠️ gotcha | `.cursor/rules/<технология>-gotcha.mdc` |
| 🎯 skill | `.cursor/rules/<workflow>-skill.mdc` |
| 🪝 hook (spec) | `.cursor/rules/<hook-name>-hook.mdc` |
| 📁 project-specific | `.cursor/rules/<domain>-<name>.mdc` |

**ВАЖНО:** БЕЗ подпапок! Все `.mdc` файлы напрямую в `.cursor/rules/`.
...
| Триггер в сессии | Файл правила |
|------------------|--------------|
| "напрямую в prod" | `no-direct-prod-edits.mdc` |
| "UPDATE ... WHERE" на проде | `no-manual-sql-prod.mdc` |
| "захардкодил" | `no-hardcoded-values.mdc` |
| "костыль", "workaround" | `document-tech-debt.mdc` |
| "потом исправлю" | `no-deferred-fixes.mdc` |
| "скопировал код" | `no-copy-paste.mdc` |
| "отключил тесты" | `no-disabled-tests.mdc` |
| "игнорируем ошибку" | `no-silent-catch.mdc` |
| "try {} catch {}" без обработки | `no-empty-catch.mdc` |
| "без миграции" | `always-use-migrations.mdc` |
| "руками проверил" | `automate-checks-checklist.mdc` |
| "повторял команду 3+ раз", "исправил команду с N-й попытки" | `command-retry-gotcha.mdc` |
| "curl ... invalid argument", "curl: (3) URL malformed" | `curl-args-checklist.mdc` |
| "нет, делай так", "я же говорил", "не так" | `user-correction-rule-gap.mdc` |
| "опять спрашиваю", "снова не помню" | `undocumented-convention.mdc` |
| "нарушил правило", "по правилу надо" | `rule-clarity-improvement.mdc` |
| "пришлось обойти", "workaround" | `missing-documentation.mdc` |
| "сделай как в прошлый раз", "опять то же самое" | `repeatable-workflow-skill.mdc` |
| "ты запустил тесты?", "ты проверил?" | `auto-verification-hook.mdc` |
| "каждый раз проверяю вручную" | `manual-gate-automation-hook.mdc` |
| "опять упала команда", "снова ошибка в..." | `command-failure-validator-hook.mdc` |
...
```markdown
| "<фраза из сессии>" | `<kebab-name>.mdc` |
```
...
> suggest-rules детектирует 7 типов триггеров. Триггеры T1-T6 обнаруживаются в Phase 1 (Session Analysis).
...
| T4 | Rule Violation | High | Claude нарушил правило из `.cursor/rules/` | 🟢 pattern (уточнение) |
...
8. **MDC формат** — YAML frontmatter обязателен (name, description, alwaysApply)
...
10. **НИКОГДА не пропускай pipeline из-за размера контекста** — модель обязана выполнить ВСЕ фазы. Запрещено: "контекст слишком длинный", "сессию нужно завершить", "сокращённый анализ". Если MCP/insights недоступны — пропускай только эти конкретные шаги, остальные фазы выполняй полностью
...
7. [TEXT] Phase 1: анализ сессии (сырые находки + self-improving signals + skill/hook signals)
8. [TEXT] Phase 1.5: Abstraction (извлечение antipatterns + DECISION TREE: rule/skill/hook)
...
4. Phase 0 (дерево) → Phase 0.5 (domains) → Phase 1 (сырые находки) → **Phase 1.5 (ABSTRACTION!)** → Phase 2 (category scoring + пруфы) → Phase 2.5 (merge) → Phase 3 (таблицы)
4. **СТОП** — ждать выбор
