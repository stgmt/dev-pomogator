...
## Phase 2: Quality Filter — STRUCTURED CHECK

**Для каждого кандидата (rule ИЛИ antipattern) ответь на 6 вопросов:**

| # | Вопрос | Варианты | Баллы |
|---|--------|----------|-------|
| Q1 | Применим к разным стекам/проектам? | да/частично/нет | 20/10/0 |
| Q2 | Абстрагирован от конкретных имён? | да/частично/нет | 15/7/0 |
| Q3 | Сколько раз встречался в сессии? | 3+/1-2/0 | 15/10/5 |
| Q4 | Предотвращает реальную проблему? | да/возможно/нет | 20/10/0 |
| Q5 | Понятен без контекста сессии? | да/частично/нет | 15/7/0 |
| Q6 | Не дублирует существующие? | нет/частично/да | 15/7/0 |
...
|---------|-------|----------|
| 🟢 HIGH | 90-100 | Автоматически рекомендовать |
| 🟡 MEDIUM | 70-89 | Показать для выбора |
| 🟠 LOW | 50-69 | Показать с предупреждением |
| 🔴 REJECT | <50 | Отклонить |
...
✅/❌ ОЦЕНКА:
├── Q1 Универсальный? <ответ> (+<баллы>)
│   └── <пояснение с пруфом>
├── Q2 Абстрактный? <ответ> (+<баллы>)
│   └── <пояснение с пруфом>
├── Q3 Частота? <ответ> (+<баллы>)
├── Q4 Предотвращает проблему? <ответ> (+<баллы>)
│   └── <пояснение>
├── Q5 Понятен без контекста? <ответ> (+<баллы>)
├── Q6 Не дублирует? <ответ> (+<баллы>)
└── CONFIDENCE: <N>% <LEVEL>
...
**Для каждого кандидата с confidence ≥50%:**
...
3. **Анализ совпадения:** Read похожий файл и определить тип совпадения
...
| 🔀 MERGE | 30-80% | Дополнить существующий |
| ⚠️ DUP | >80% | Пропустить (уже есть) |
...
### 🟢 Рекомендовано (HIGH confidence)

| # | Название | Тип | Score | Источник | Статус |
|---|----------|-----|-------|----------|--------|
| 1 | <name>   | 🔴 antipattern | 95% | 📍 turn #12 | 🆕 NEW |
| 2 | <name>   | 🟢 pattern | 92% | 🧠 #163 | 🔀 MERGE |

### 🟡 Для выбора (MEDIUM confidence)

| # | Название | Тип | Score | Источник | Статус |
|---|----------|-----|-------|----------|--------|
| 3 | <name>   | 📋 checklist | 78% | 📍 turn #5 | 🆕 NEW |

### 🔴 Отклонены (LOW/REJECT)

| - | Название | Score | Причина |
|---|----------|-------|---------|
| - | <name>   | 45%   | Q1: слишком специфично |
...
---
**Выбор:** `1,2,3,...` | `all` | `recommended` | `0` (отмена)
```
...
9. [TEXT] Phase 2: Quality Filter (Q1-Q6, confidence, ПРУФЫ)
...
3. Phase 0 (дерево) → Phase 1 (сырые находки) → **Phase 1.5 (ABSTRACTION!)** → Phase 2 (Q1-Q6 + пруфы) → Phase 2.5 (merge) → Phase 3 (таблицы)
4. **СТОП** — ждать выбор
