...
## Phase 2: Quality Filter — CATEGORY-BASED
...
**PREVENTS подсказка:** если ошибка заняла >5 минут — считать полным.
...
| Критерий | Вес | Подсказка |
|----------|-----|----------|
| FREQUENCY | 40 | Как часто задача встречается в проекте |
...
**FREQUENCY шкала:** 3+ = 100%, 1-2 = 50%, 0 = 0%.
**Подсказка:** 3+ попытки одной команды в сессии = 3+.
...
9. [TEXT] Phase 2: Quality Filter (category scoring, confidence, ПРУФЫ)
