...
### Сейчас (как работает)
Каждый разработчик пишет планы в своём формате. Проверять консистентность приходится вручную, и это легко пропустить.

### Как должно быть (как я понял)
Скрипт-валидатор автоматически проверяет структуру плана, ругается на отсутствующие секции и подсказывает что именно нужно добавить. Запускается одной командой.

### Правильно понял?
Да, валидатор должен быть простой CLI-утилитой с понятными actionable hints в выводе.
