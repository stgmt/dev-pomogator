...
- **Gitmoji Support**: Commit messages with semantic emojis (✨ feat, 🐛 fix, etc.) in full format
...
📊 Tokens: 500 (prompt: 400, completion: 100)
```
...
├── extension.json           # Hook configuration
├── README.md                # This file
└── tools/auto-commit/
    ├── auto_commit_core.ts      # Config, state, git operations
    ├── auto_commit_llm.ts       # LLM API client
    ├── auto_commit_transcript.ts # Transcript parsing
    ├── auto_commit_stop.ts      # Main entry point
    └── auto_commit_prompt_guide.md # Prompt guidelines
```
