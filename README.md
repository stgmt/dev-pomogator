...
### Защита `.specs/` у пользователей плагина

Плагин включает `spec-access-guard` через `userConfig.spec_access_enforce` (по
умолчанию `true`). Он направляет чтение и запись спек в MCP-дверь и отдельно
блокирует неявные whole-tree мутации Git (`git add -A`, `git add --all`,
`git commit -a`/`-am`). Обычный commit message, в котором путь лишь упомянут как
текст, не блокируется.

Формат plugin manifest не позволяет плагину добавлять пользовательские
`permissions.deny`. Для независимого permissions-бэкстопа добавьте в проектный
...
Этот слой дополняет hook, но не заменяет его: permissions-glob не распознаёт
whole-tree Git-команды, тогда как guard распознаёт.
