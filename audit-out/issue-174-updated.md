...
## Выполненный первый срез (2026-07-23)

- Исправлен live false-green в `get_spec_status(view=coverage)`: FR остаётся в `FR_NOT_EXECUTION_VERIFIED`, пока **каждый** связанный сценарий не имеет bucket `passed`.
- Добавлен Docker-BDD regression `SPECGEN004_544` для смеси PASSED + FAILED + NOT_RUN.
- Это узкий prerequisite; полная canonical-verdict консолидация из этого issue ещё не завершена.
