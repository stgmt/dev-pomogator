/**
 * One-shot FR-81 authoring via the real MCP mutation door.
 * Run: node --import tsx audit-out/fr81-apply.ts
 */
...
async function main(): Promise<void> {
  ...
  async function call(name: string, args: Record<string, unknown>): Promise<void> {
    ...
    if (errors.length) {
      console.error(`FINDINGS ${name}`, JSON.stringify(errors, null, 2).slice(0, 4000));
...
`,
  ...
  console.log('FR-81 core docs applied');
}
