/**
 * FR-81 remaining txn only (README/CHANGELOG already applied).
 * Run: node --import tsx audit-out/fr81-rest2.ts
 */
...
async function main(): Promise<void> {
  ...
  async function call(name: string, args: Record<string, unknown>): Promise<void> {
    ...
    if (!parsed.ok) {
      console.error(`FAIL ${name}`, JSON.stringify(parsed, null, 2).slice(0, 8000));
      throw new Error(`${name} failed`);
    }
    console.log(`OK ${name}`);
  }

  await call('apply_spec_transaction', {
    reason: REASON,
    edits: [
      {
        spec: SPEC,
...
`,
  ...
  console.log('FR-81 rest2 txn done');
}
