...
const data = JSON.parse(raw.slice(jsonStart)) as { timestamp?: string; findings?: Array<{ check: string; category: string; severity: string; message: string; details?: string }> };
const f = data.findings || [];
const bySev: Record<string, number> = {};
const byCheckSev: Record<string, number> = {};
for (const x of f) {
...
console.log('top-level:', Object.keys(data).filter(k => k !== 'findings').map(k => `${k}=${JSON.stringify((data as Record<string, unknown>)[k])}`).join(' | '));
