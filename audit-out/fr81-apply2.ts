/**
 * FR-81 pass 2: FR-81 first, then sibling docs, then cross-links.
 * Run: node --import tsx audit-out/fr81-apply2.ts
 */
...
const REASON =
  'FR-81 Cursor compat-first [skip-spec-steer: fr81 authoring via door]';
...
async function main(): Promise<void> {
  ...
  async function call(name: string, args: Record<string, unknown>): Promise<void> {
    ...
    if (parsed.ok === false || parsed.error) {
      console.error(`FAIL ${name}`, JSON.stringify(parsed, null, 2).slice(0, 6000));
      throw new Error(`${name} failed`);
    }
    const errors = (parsed.findings ?? []).filter((f) => f.severity === 'error');
    if (errors.length) {
      console.error(`FINDINGS ${name}`, JSON.stringify(errors, null, 2).slice(0, 6000));
      throw new Error(`${name} findings`);
    }
    console.log(`OK ${name} ${String(args.doc ?? '')}`);
  }

  async function apply(doc: string, old_string: string, new_string: string): Promise<void> {
    await call('apply_spec_change', { spec: SPEC, doc, old_string, new_string, reason: REASON });
  }

  async function insertEof(doc: string, text: string): Promise<void> {
    await call('insert_at_eof', { spec: SPEC, doc, text, reason: REASON });
  }
...
**Связанные AC:** AC-81.1 through AC-81.6
**Use Case:** UC-33
**User Story:** User Story 61
`,
  );
