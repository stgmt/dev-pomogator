 ...
 */
export async function backupUserFile(projectPath, relativePath) {
    try {
        ...
        if (!await fs.pathExists(sourcePath))
            return null;
        const backupPath = resolveWithinDir(path.join(projectPath, USER_OVERRIDES_DIR), relativePath);
        if (!backupPath) {
 ...
 */
export async function writeUpdateReport(modifications) {
