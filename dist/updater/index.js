...
import { RULES_SUBFOLDER } from '../constants.js';
...
async function updateCommandFiles(extensionName, platform, files, projectPath, previousItems) {
    if (files.length === 0)
    ...
    for (const relativePath of files) {
        ...
        const relativeDest = normalizeRelativePath(path.join(platformDir, 'commands', fileName));
        if (!content) {
...
async function updateRuleFiles(extensionName, platform, files, projectPath, previousItems) {
    if (files.length === 0)
        return { written: [], hadFailures: false, backedUp: [] };
    const platformDir = platform === 'claude' ? '.claude' : '.cursor';
    ...
    for (const relativePath of files) {
        ...
        const fileName = path.basename(relativePath);
        const destFile = path.join(destDir, fileName);
        const relativeDest = normalizeRelativePath(path.join(platformDir, 'rules', RULES_SUBFOLDER, fileName));
        if (!content) {
...
async function updateCursorHooksForProject(repoRoot, hooks, previousManagedHooks = {}) {
    ...
    for (const eventName of Object.keys(existingHooks.hooks)) {
        existingHooks.hooks[eventName] = existingHooks.hooks[eventName].filter(h => !h.command.includes('.dev-pomogator/tools/')
    ...
    for (const [eventName, commands] of Object.entries(nextAbsoluteByEvent)) {
        ...
        for (const escapedCommand of commands) {
            const exists = existingHooks.hooks[eventName].some((h) => h.command === escapedCommand);
            if (!exists) {
    ...
    for (const [eventName, commands] of Object.entries(previousManagedHooks)) {
        ...
        const previousAbsolute = commands.map((cmd) => resolveHookToolPaths(cmd, repoRoot).replace(/\\/g, '\\\\'));
...
async function updateClaudeHooksForProject(repoRoot, hooks, previousManagedHooks = {}) {
    ...
    for (const [hookName, rawHook] of Object.entries(hooks)) {
        ...
        const commandExists = hookArray.some((h) => h.hooks?.some((hook) => hook.command === command));
        if (!commandExists) {
            hookArray.push({
                matcher,
                hooks: [{
    ...
    for (const [hookName, commands] of Object.entries(previousManagedHooks)) {
        ...
        const nextAbsolute = new Set((nextManagedHooks[hookName] ?? []).map(cmd => resolveHookToolPaths(cmd, repoRoot)));
        ...
        existingHooks[hookName] = hookArray.filter((entry) => !entry.hooks?.some((hook) => removeSet.has(hook.command)));
    }
            ...
            try {
                ...
                for (const projectPath of installed.projectPaths) {
                    try {
                        ...
                        const commandResult = await updateCommandFiles(installed.name, installed.platform, commandFiles, projectPath, managedEntry.commands);
                        const ruleResult = await updateRuleFiles(installed.name, installed.platform, ruleFiles, projectPath, managedEntry.rules);
                        const toolResult = await updateToolFiles(installed.name, toolFiles, projectPath, managedEntry.tools);
                        const skillResult = await updateSkillFiles(installed.name, skillFiles, projectPath, managedEntry.skills);
                        // Collect backed-up files
                        allBackedUp.push(...commandResult.backedUp, ...ruleResult.backedUp, ...toolResult.backedUp, ...skillResult.backedUp);
                        ...
                        allBackedUp.push(...await removeStaleFiles(projectPath, managedEntry.commands, writtenCommandPaths, installed.name));
                        managedEntry.commands = commandResult.written;
                        allBackedUp.push(...await removeStaleFiles(projectPath, managedEntry.rules, writtenRulePaths, installed.name));
                        managedEntry.rules = ruleResult.written;
                        allBackedUp.push(...await removeStaleFiles(projectPath, managedEntry.tools, writtenToolPaths, installed.name));
                        managedEntry.tools = toolResult.written;
                        allBackedUp.push(...await removeStaleFiles(projectPath, managedEntry.skills, writtenSkillPaths, installed.name));
