...
import { listExtensions, getExtensionFiles, getExtensionRules, getExtensionTools, getExtensionHooks, runPostInstallHook, cleanStaleNodeModulesDirs } from './extensions.js';
...
export async function installCursor(options) {
    ...
    const cursorExtensions = allExtensions.filter((ext) => ext.platforms.includes('cursor'));
 ...
 */
async function installExtensionHooks(extensions, repoRoot) {
    ...
    for (const [eventName, commands] of Object.entries(extensionHooks)) {
        ...
        for (const newCommand of commands) {
            ...
            if (!scriptName)
                continue;
            existingHooks.hooks[eventName] = existingHooks.hooks[eventName].filter(h => {
                if (h.command === escapedNew)
                    return true; // keep the new one
