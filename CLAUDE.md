...
| spec-reality-check/maintain-evals-on-edit | При изменении `spec-reality-check` skill — прогнать `run-evals.ts` + `bulk-run.ts` + `bench-synthetic.ts`; при изменении form-filler skills (`discovery-forms` / `requirements-chk-matrix` / `task-board-forms`) — прогнать их executable evals с real form-guards и P16-1 negative pins | `.claude/rules/spec-reality-check/maintain-evals-on-edit.md` |
