...
| dead-integration-guard | Диф добавляет установщик/скачиваемый бинарь/рантайм-зависимость → требуется рантайм-потребитель (не флаг/тест) + e2e против реального артефакта (silent-skip в Docker = FAIL). «Installed ≠ integrated» (инцидент FR-7 Marksman: скачан, но не использован) | `.claude/rules/testing/dead-integration-guard.md` |
