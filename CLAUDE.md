...
| no-host-bdd-runs | Полный cucumber/BDD-прогон ТОЛЬКО в Docker (`docker-bdd.sh`), не на хосте: хостовый full-run даёт ложные красные (Linux-only сценарии), 70 мин, перезаписывает канон артефактами изоляции. PreToolUse `test_guard` (ветка host-bdd) денаит full host `run-bdd.mjs`/`cucumber.js` → docker-bdd.sh; фильтрованный (`--name`) на хосте разрешён (throwaway). Инцидент 2026-06-24 | `.claude/rules/pomogator/no-host-bdd-runs.md` |
